<?php include('config/functions.inc.php');
##==validate user====
validate_user();

###request for get single project details using project id ===========
$project_id = '';
$activity_id = '';
if(isset($_SESSION['project_id']) && isset($_SESSION['activity_id']))
{
	$project_id = $_SESSION['project_id'];
	$activity_id = $_SESSION['activity_id'];
	
	$url = API_HOST_URL_PROJECT."get_project_activity.php?project_id=".$project_id."&activity_id=".$activity_id;  
	$project_activity_arr = requestByCURL($url);
}
else{
	header("Location:list_activity");
}
?> 
<!DOCTYPE html>
<html>
<head>
	<link rel="shortcut icon" type="image/ico" href="img/favicon.ico" />
	<title><?php echo TITLE;?></title>
	<?php include('includes/resources.php');?>
</head>
<body class="page-ui-components">
	<!--header start-->
	<?php include('includes/header.php');?>
	<!--header end-->
	<!--main container start-->
	<div class="main-container container-fluid" id="main-content">
		<div>
			<!--project overview start-->
			<?php include('includes/activity_header.php');?>	 
			<div class="extra_ht"></div><div class="extra_ht"></div>
			<div class="tbl-block">
				<div class="tbl-caption">
					<div class="tbl-content-head">Manage Team Member</div>
					<!-- <div class="tbl-content-btn">
						<a href="#" class="tbl-up"><i class="fa fa-chevron-up"></i></a> 
						<a href="#" class="tbl-down"><i class="fa fa-chevron-down"></i></a>
					</div> -->
					<div class="clear"></div>
				</div>
				<!--add new project start-->
				<div class="table-container">
					<div class="form-blk">
						<!--add team member block start-->
						<div>
							<button class="add_new_team" type="button"><i class="fa fa-user" aria-hidden="true"></i> Add new team member</button>
							<form id="add_new_team">
								<div>
									<div class="form-msg usa-alert disp-none">
										<div class="usa-alert-body">
											<h3 class="usa-alert-heading"></h3>
											<p class="usa-alert-text"></p>
										</div>
									</div>
								</div>
								<div class="row add_team_blk disp-none">
									<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 search_team_member">
										<div style="height:10px;"></div>
										
										<header><h3 class="form-blk-head bold paddt-20">Team Member</h3></header>
										<div class="desc-txt">
											Select the name of the person you want to add to the team
										</div>
										<div id="search_div" style="position:relative">
											<input type="hidden" name="project_id" class="project_id" value="<?php echo $project_id;?>"/>
											<input type="hidden" name="activity_id" class="activity_id" value="<?php echo $activity_id;?>"/>
											<select class="usa-form" style="background-image:none;" id="project_team_dwp">

											</select>
											<!--<input type="text" name="employee_name" id="find_team_member" class="form-control" autocomplete="off"/>-->
											<input type="hidden" name="employee_id" id="find_team_member_id" />
										</div>
									</div>
									<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
										<header><h3 class="form-blk-head bold paddt-20">Activity Role</h3></header>
										<div class="desc-txt">
											Select the person role
										</div>
										<div class="row threshold">
											<div class="col-lg-5 col-md-6 col-sm-6 col-xs-12">
												<div>
													<div class="div_label">
														<input type="radio" name="project_team_role" class="team_role" onClick="show_save_btn()" value="Project Manager"/> <label>Project Manager</label>
													</div>
													<div class="extra_ht"></div>
													<div class="div_label">
														<input type="radio" name="project_team_role" class="team_role" onClick="show_save_btn()" value="COR/AOR"/> <label>COR/AOR</label>	
													</div>
													<div class="extra_ht"></div>
													<div class="div_label">
														<input type="radio" name="project_team_role" class="team_role" onClick="show_save_btn()" value="CO"/> <label>CO</label>
													</div>
													<div class="extra_ht"></div>
													<div class="div_label">
														<input type="radio" name="project_team_role" class="team_role" onClick="show_save_btn()" value="Financial Advisor"/> <label>Financial Advisor</label>
													</div>
													<div class="extra_ht"></div>
													<div class="div_label">
														<input type="radio" name="project_team_role" class="team_role" onClick="show_save_btn()" value="Technical Advisor"/> <label> Technical Advisor</label>
													</div>
												</div>
											</div>
											<div class="col-lg-7 col-md-6 col-sm-6 col-xs-12">
												<div>
													<div class="div_label">
														<input type="radio" name="project_team_role" class="team_role" onClick="show_save_btn()" value="Legal Advisor"/> <label>Legal Advisor</label>
													</div>
													<div class="extra_ht"></div>
													<div class="div_label">
														<input type="radio" name="project_team_role" class="team_role" onClick="show_save_btn()" value="Communications Advisor"/> <label> Communications Advisor</label>
													</div>
													<div class="extra_ht"></div>
													<div class="div_label">
														<input type="radio" name="project_team_role" class="team_role" onClick="show_save_btn()" value="Project Administration"/> <label> Project Administration</label></div><div class="extra_ht"></div>
														<div class="div_label">
															<input type="radio" name="project_team_role" class="team_role" onClick="show_save_btn()" value="Quality Assurance"/> <label>Quality Assurance</label>
														</div>
													</div>
												</div>
												<div class="clear"></div>
											</div>
										</div>
										<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 search_team_member">
											<header><h3 class="form-blk-head bold paddt-20">Start Date</h3></header>	
											<div class="desc-txt">
												Date the person starts on the project
											</div>
											<div class="calendar-blk" style="margin-bottom:20px">
												<table class="project_dates no-bdr">
													<tr class="head">
														<td>Month</td>
														<td>Day</td>
														<td>Year</td>
													</tr>
													<tr>
														<td><input type='text' class="form-control month date_ip only_num" value="" onKeyup="show_save_btn()" placeholder="MM" /></td>
														<td><input type='text' class="form-control date date_ip only_num" value="" onKeyup="show_save_btn()" placeholder="DD" /></td>
														<td><input type='text' class="form-control year date_ip only_num" value="" onKeyup="show_save_btn()" placeholder="YYYY" /></td>
													</tr>
												</table>
												<input type="hidden" name="team_member_start_date" class="formatted_date" value=""/>
											</div>
											<input type="hidden" value="add_project_team" name="add_project_team"/>
											<button class="usa-button-outline cancel_team" type="button">Cancel</button>
											<button class="save_team" type="button">Save</button>
										</div>
									</div>
									<div class="gray-line"></div>
								</form>
							</div>
							<!--add team member block end-->
							<!--edit team member block start-->
							<div class="edit_team_member_blk disp-none">
								<form id="edit_team_member_info">
									<div class="form-msg usa-alert disp-none">
										<div class="usa-alert-body">
											<h3 class="usa-alert-heading"></h3>
											<p class="usa-alert-text"></p>
										</div>
									</div>
									<div class="row">
										<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 search_team_member">
											<header><h3 class="form-blk-head bold paddt-20">Edit Team Member</h3></header>
											<div style="height:10px;"></div>
											<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 img-blk">
												<img src="img/user.png"  width="130" class="center-block emp_img"/>
												<input type="hidden" name="project_id" class="project_id" value="<?php echo $project_id;?>"/>
												<input type="hidden" name="activity_id" class="activity_id" value="<?php echo $activity_id;?>"/>
												<input type="hidden" class="emp_id" name="emp_id"/>
												<input type="hidden" class="emp_old_role" name="emp_old_role"/>
												<div class="user-info">
													<div class="emp_name"</div>
													</div>
												</div>
											</div>
										</div>
										<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
											<header><h3 class="form-blk-head bold paddt-20">Activity Role</h3></header>
											<div class="desc-txt">
												Select the person role
											</div>
											<div class="row threshold">
												<div class="col-lg-5 col-md-6 col-sm-6 col-xs-12">
													<div class="div_label">
														<input type="radio" name="project_team_role" class="project_team_role" onClick="show_save_btn()" value="Project Manager"/> <label>Project Manager</label>
														
													</div>	
													<div class="extra_ht"></div>
													<div class="div_label">
														<input type="radio" name="project_team_role" class="project_team_role" onClick="show_save_btn()" value="COR/AOR"/> <label>COR/AOR</label>
													</div>
													<div class="extra_ht"></div>
													<div class="div_label">
														<input type="radio" name="project_team_role" class="project_team_role" onClick="show_save_btn()" value="CO"/> <label>CO</label>
													</div>
													<div class="extra_ht"></div>
													<div class="div_label">
														<input type="radio" name="project_team_role" class="project_team_role" onClick="show_save_btn()" value="Financial Advisor"/> <label>Financial Advisor</label>
													</div>
													<div class="extra_ht"></div>
													<div class="div_label">
														<input type="radio" name="project_team_role" class="project_team_role" onClick="show_save_btn()" value="Technical Advisor"/> <label>Technical Advisor</label>
													</div>
												</div>
												<div class="col-lg-7 col-md-6 col-sm-6 col-xs-12">
													<div class="div_label">
														<input type="radio" name="project_team_role" class="project_team_role" onClick="show_save_btn()" value="Legal Advisor"/> <label>Legal Advisor</label>
													</div><div class="extra_ht"></div>
													<div class="div_label">
														<input type="radio" name="project_team_role" class="project_team_role" onClick="show_save_btn()" value="Communications Advisor"/> <label>Communications Advisor</label>
													</div><div class="extra_ht"></div>
													<div class="div_label">
														<input type="radio" name="project_team_role" class="project_team_role" onClick="show_save_btn()" value="Project Administration"/> <label>Project Administration</label>
													</div><div class="extra_ht"></div>
													<div class="div_label"><input type="radio" name="project_team_role" class="project_team_role" onClick="show_save_btn()" value="Quality Assurance"/> <label>Quality Assurance</label>
													</div>
												</div>
											</div>
										</div>
										<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 search_team_member">
											<input type="hidden" value="edit_project_team" name="edit_project_team"/>
											<button class="noupdate_in_team usa-button-outline" type="button">Cancel</button>
											<button class="update_team disp-none" type="button">Save</button>
										</div>
									</div>
								</form>
							</div>
							<!--edit team member block end-->
							<!--add current team member block start-->
							<div class="current_team">
								<header><h3 class="form-blk-head bold paddt-20">Current Team</h3></header>
								<div class="form-msg usa-alert disp-none">
									<div class="usa-alert-body">
										<h3 class="usa-alert-heading"></h3>
										<p class="usa-alert-text"></p>
									</div>
								</div>
								<div class="row current_team_data">
									<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12"><div class="gray-line"></div></div>
								</div>
							</div>
							<!--add current team member block end-->
							<!--old team start-->
							<button class="hide_old_team_btn" type="button">Hide team history</button>
							<button class="usa-button-gray show_old_team_btn disp-none" type="button">Show team history</button>	
							<div class="table-container">
								<div class="form-blk">
									<!--old team member block start-->
									<div class="old_team_blk">	
										<header><h3 class="form-blk-head bold paddt-20">Team History</h3></header>
										<div class="row old_team_data">
											<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12"><div class="gray-line"></div></div>
										</div>
									</div>
								</div>		
							</div>
						</div>		
					</div>
					<!--add new project end-->
				</div>
			</div>
			<!--add new project start-->
		</div>
		<!--main container end-->
		<?php include('includes/footer.php');?>
		<script src="<?php echo HOST_URL?>js/main.js"></script>
		<script src="<?php echo HOST_URL?>js/manage_team_member.js"></script>	
		<script>
		$('.save_team').click(function(){
			var form_data = $('#add_new_team').serialize();
			$('.form-msg').removeClass('success error');
			if($('#add_new_team').find('.invalid_ip').length==0){
				$.ajax({
					url:'<?php echo HOST_URL?>ajaxfiles/manage_project_activity_team.php',
					type:'POST',
					data: form_data,
					success:function(data){
						var project_id = $('.project_id').val();
						var activity_id = $('.activity_id').val();
						
						var data = JSON.parse(data);
						reset_add_new_team_form();
						$('.save_team').addClass('disp-none');
						
						
						$('#add_new_team').find('.usa-alert-text').text(data['msg']);
						
						if(data['msg_type']=='Success'){
							$('#add_new_team').find('.usa-alert-heading').text('Success');
							$('#add_new_team').find('.form-msg').addClass('usa-alert-success').removeClass('disp-none');
						}
						else{
							$('#add_new_team').find('.usa-alert-heading').text('Error');
							$('#add_new_team').find('.form-msg').addClass('usa-alert-error').removeClass('disp-none');
						}
						get_current_team(project_id,activity_id);
						get_old_team(project_id,activity_id);
						setTimeout(function(){
							$('#add_new_team').find('.form-msg').addClass("disp-none").removeClass('usa-alert-success usa-alert-error');
						},2500);
					}
				});
			}
			else{
				$('#add_new_team').find('.form-msg').html("Something went wrong...");
				$('#add_new_team').find('.form-msg').addClass('error');
				$(document).scrollTop(0);
				setTimeout(function(){
					$('#add_new_team').find('.form-msg').html("");
				},5000);
			}
		});
	
	function get_current_team(project_id,activity_id){
		$.ajax({
			url:'<?php echo HOST_URL?>ajaxfiles/manage_project_activity_team.php',
			type:'POST',
			data: {'get_project_team':'get_project_team',project_id:project_id,activity_id:activity_id},
			success:function(result){
				$('.current_team_data').html(result);
			}
		});
	}
	
	function get_old_team(project_id,activity_id){
		$.ajax({
			url:'<?php echo HOST_URL?>ajaxfiles/manage_project_activity_team.php',
			type:'POST',
			data: {'get_old_project_team':'get_old_project_team',project_id:project_id,activity_id:activity_id},
			success:function(result){
				$('.old_team_data').html(result);
			}
		});
	}
	
	/*load team member*/
	get_current_team('<?php echo $project_id;?>','<?php echo $activity_id;?>');
	
	/*load team member*/
	get_old_team('<?php echo $project_id;?>','<?php echo $activity_id;?>');
	
	/*remove team member*/
	$(document).on('click','.remove_team_member',function(){
		var form_data = $(this).closest('form').serialize();
		var project_id = $('.project_id').val();
		var activity_id = $('.activity_id').val();
		var emp_name =$(this).closest('form').find('.emp_name').val();
		reset_add_new_team_form();
		reset_edit_teammember_form();
		$('.edit_team_member_blk').addClass('disp-none');
		var msg= "Are you sure to remove "+emp_name+" from activity?";
		if(confirm(msg)){
			$.ajax({
				url:'<?php echo HOST_URL?>ajaxfiles/manage_project_activity_team.php',
				type:'POST',
				data: form_data +'&remove_team_member=remove_team_member'+'&project_id='+project_id+'&activity_id='+activity_id,
				context:this,
				success:function(data){
					var project_id = $('.project_id').val();
					var data = JSON.parse(data);
					$('.current_team').find('.form-msg').find('.usa-alert-text').text(data['msg']);
					if(data['msg_type']=='Success'){
						$('.current_team').find('.form-msg').find('.usa-alert-heading').text('Success');
						$('.current_team').find('.form-msg').addClass('usa-alert-success').removeClass('disp-none');
					}
					else{
						$('.current_team').find('.form-msg').find('.usa-alert-heading').text('Error');
						$('.current_team').find('.form-msg').addClass('usa-alert-error').removeClass('disp-none');
					}
				
					get_current_team(project_id,activity_id);
					get_old_team(project_id,activity_id);
					setTimeout(function(){
						$('.current_team').find('.form-msg').addClass("disp-none").removeClass('usa-alert-success usa-alert-error');;
					},5000);
				}
			});
		}
	});
	
	/*edit team member*/
	$(document).on('click','.edit_team_member',function(){
		reset_add_new_team_form();
		$('.add_team_blk').addClass('disp-none');
		$('.edit_team_member_blk').removeClass('disp-none');
		var emp_id = $(this).closest('.team_member_info_form').find('.emp_id').val();
		$('.edit_team_member_blk').find('.emp_id').val(emp_id);
		var name = $(this).closest('.team_member_info_form').find('.emp_name').val();
		$('.edit_team_member_blk').find('.emp_name').text(name);
		var emp_role = $(this).closest('.team_member_info_form').find('.emp_role').val();
		$('.edit_team_member_blk').find('.emp_old_role').val(emp_role);
		var team_role = $('.edit_team_member_blk').find('.project_team_role');
		var emp_img = $(this).closest('.team_member_info_form').find('.emp_img').val(); 
		$('.edit_team_member_blk').find('.emp_img').attr('src',emp_img);
		$(team_role).each(function(index, elem){
			if($(elem).val()==emp_role){
				$(elem).closest('label').css({'display':'none'});
				$(elem).closest('label').next('.extra_ht').addClass('disp-none');
				return false;
			}
		});
	});
	
	$('.project_team_role').click(function(){
		$('.update_team').removeClass('disp-none');
	});
	
	$('.noupdate_in_team').click(function(){
		$('.update_team').addClass('disp-none');
		reset_edit_teammember_form();
		$('.edit_team_member_blk').addClass('disp-none');
	});
	
	$(document).on('click','.update_team',function(){
		var form_data = $('#edit_team_member_info').serialize();
		var project_id = $('#edit_team_member_info').find('.project_id').val();
		var activity_id = $('#edit_team_member_info').find('.activity_id').val();
		$.ajax({
			url:'<?php echo HOST_URL?>ajaxfiles/manage_project_activity_team.php',
			type:'POST',
			data: form_data,
			success:function(data){
				var project_id = $('.project_id').val();
				var data = JSON.parse(data);
				$('#edit_team_member_info').find('.form-msg').find('.usa-alert-text').text(data['msg']);
				if(data['msg_type']=='Success'){
					$('#edit_team_member_info').find('.form-msg').find('.usa-alert-heading').text('Success');
					$('#edit_team_member_info').find('.form-msg').addClass('usa-alert-success').removeClass('disp-none');
					$('.noupdate_in_team').addClass('disp-none');
					reset_edit_teammember_form();
				}
				else{
					$('#edit_team_member_info').find('.form-msg').find('.usa-alert-heading').text('Error');
					$('#edit_team_member_info').find('.form-msg').addClass('usa-alert-error').removeClass('disp-none');
				}
				
				get_current_team(project_id,activity_id);
				setTimeout(function(){
					$('#edit_team_member_info').find('.form-msg').removeClass('usa-alert-success usa-alert-error').addClass('disp-none');
					$('.edit_team_member_blk').addClass('disp-none');
					$('.noupdate_in_team').removeClass('disp-none');
				},3000);
			}
		});
	});
	
	$('.add_new_team').click(function(){
		var project_id = $('.project_id').val();
		$('#project_team_dwp').html("<option value=''>Select</option>");
		$.ajax({
			url:'<?php echo HOST_URL?>ajaxfiles/manage_project_activity_team.php',
			type:'POST',
			data:{'get_project_activity_team':'get_project_activity_team',project_id:project_id},
			success:function(data){
				$('#project_team_dwp').html("");
				$('#project_team_dwp').append(data);
			}
		});	
	});
	
	$(document).on('change','#project_team_dwp',function(){
		var val = $(this).val();
		if(val!=""){
			$('#find_team_member_id').val(val);
		}
	});
</script>
	</body>
	</html>
