...
$url = API_HOST_URL."get_all_sub_obligate_fund_group.php"; //api for get group data for list
...
if(isset($_REQUEST['obligate_fund']))
{
	...
	$ledger_type_cr = 'Award CLIN'; //like ledger type cr
	$ledger_type_id_cr = trim($_REQUEST['commit_in_id']); //like sub-obligate id cr
	...
	else
	{
		...
		if($transaction_id>0)
		{
			### get oppening and closing balance of cr entry like project id and activity etc=====
		 	$opn_clg_balance_arr = getClosingBalance($ledger_type_id_cr,$beginning_fiscal_year,$ending_fiscal_year,$fund_id,$ledger_type_cr,$pe_id,'Subobligate');
		 	...
		 	$opn_clg_balance_arr = getClosingBalance($ledger_type_id_dr,$beginning_fiscal_year,$ending_fiscal_year,$fund_id,$ledger_type_dr,$pe_id,'Obligate');
		 	$opening_balance = $opn_clg_balance_arr['opening_balance'];
		 	$closing_balance = $opn_clg_balance_arr['closing_balance'];
		 	...
		 	{
		 		header("location:sub_obligate_funds.php");
		 	}
		 }
	}
}
...
if(isset($_REQUEST['reverse__to_unobligate']))
{
	...
	$fund_amount = getNumericAmount($fund_amount); // use this function if amount has $ sign or comma
	$ledger_type_dr = trim($_REQUEST['ledger_type']);
	...
	if($fund_amount>0)
	{
		...
		$opn_clg_balance_arr = getClosingBalance($ledger_type_cr_id,$beginning_fiscal_year,$ending_fiscal_year,$fund_id,$ledger_type_cr,$pe_id,'Obligate');
		$opening_balance = $opn_clg_balance_arr['opening_balance'];
	 	$closing_balance = $opn_clg_balance_arr['closing_balance'];
	 	$opening_balance =  $closing_balance;
	 	$closing_balance = ($closing_balance + $fund_amount);
	 	...
	 	###insert query for debit amount into transaction table======================= 	
		$opn_clg_balance_arr = getClosingBalance($debit_from_id,$beginning_fiscal_year,$ending_fiscal_year,$fund_id,$ledger_type_dr,$pe_id,'Subobligate');
		$opening_balance = $opn_clg_balance_arr['opening_balance'];
	 	$closing_balance = $opn_clg_balance_arr['closing_balance'];
...
<body>
	...
	<ol class="breadcrumb">
  		...
  		<li class="active">Sub-Obligate Funds</li>
	</ol>
			...
			<div class="manage-funds"> 
				<form class="form-horizontal" role="form" method="post" action="">  
					<div class="form-group">
						...
						<div class="col-sm-8">
							<select class="form-control operating_unit_id" name="operating_unit_id" onchange="getInputFieldByCommitType();">
					...
					<div class="form-group">
						...
						<div class="col-md-8 show_ou_fund">
							<?php
							$url = API_HOST_URL."get_all_obligated_ledger_fund_for_dropdwon.php"; 
							...
							<select class="form-control strip_year" name="strip_year" onchange="getcurrentFiscalyear();">
								...
								{ 
									$unique_total_fund_arr = getClosingBalance($unique_fund_arr['data'][$i]['ledger_type_id_cr'],$unique_fund_arr['data'][$i]['fund_beginning_fiscal_year'],$unique_fund_arr['data'][$i]['fund_ending_fiscal_year'],$unique_fund_arr['data'][$i]['fund_id'],$unique_fund_arr['data'][$i]['ledger_type'],$unique_fund_arr['data'][$i]['program_element_id'],'Obligate');
								?>
	...
	<div style="float:right;margin-bottom:5px;">
			 <table >
			 	<tr>
			 		<td style="background:red;width:20px;">&nbsp;</td><td>&nbsp; De-SubObligated &nbsp;&nbsp;</td> 
		...
		<table id="manage-table" class="table table-striped table-bordered" cellspacing="0" width="100%">
	        ...
	        { 
	            ...
	            { 
					...
					$closing_balance_arr = getClosingBalance($ledger_type_id,$fund_beginning_fiscal_year,$fund_ending_fiscal_year,$fund_id,$ledger_type,$pe_id,'Subobligate');
					
					### get UnObligated=============
					$ledger_type = str_replace(" ","_",$ledger_type);
					$url = API_HOST_URL."get_all_transaction_de_sub_obligated_count.php?ledger_type_id=".$ledger_type_id."&b_year=".$fund_beginning_fiscal_year."&e_year=".$fund_ending_fiscal_year."&fund_id=".$fund_id."&pe_id=".$pe_id."&ledger_type=".$ledger_type.""; 
					$de_sub_obligated_balance_arr = requestByCURL($url);
					$total_de_sub_obligated = $de_sub_obligated_balance_arr['data'][0]['total_debit_amount']; 
					...
					$url = API_HOST_URL."get_all_transaction_sub_obligated_to_account_payable_count.php?ledger_type_id=".$ledger_type_id."&b_year=".$fund_beginning_fiscal_year."&e_year=".$fund_ending_fiscal_year."&fund_id=".$fund_id."&pe_id=".$pe_id."&ledger_type=".$ledger_type.""; 
					$alloted_balance_arr = requestByCURL($url);
					$total_paid = $alloted_balance_arr['data'][0]['total_amount']; 
										
					### get all fund source of this ===========unique_fund_arr
					$url = API_HOST_URL."get_all_obligated_group_source.php?ledger_type_id=".$ledger_type_id."&b_year=".$fund_beginning_fiscal_year."&e_year=".$fund_ending_fiscal_year."&fund_id=".$fund_id."&program_element_id=".$pe_id."&ledger_type=".$ledger_type;
		            ...
		            <tr class="parent-tbl">
		            	...
		            	<td><?php echo $ledger_type_id.' ('.$all_obligate_fund_gp_arr['data'][$count_pe]['ledger_type'].')'; ?></td> 
		            	...
		            	<td><?php echo '$'.number_format($total_de_sub_obligated); ?></td>
		                ...
		                <form action="" method="post">
	            			...
	            			<div class="rev-amt disp-none">		  
		                		<p><b>Enter DeSubObligate Amount</b></p>
		                		<div class="row">
		                			<div class="col-md-12" >
		                	...
		                	<div class="rev"> 
		            			 ...
		            			 <input type="button"   value="DeSubObligate" class="btn btn-default" onClick="reverse(this)">
		            		</div>
	            		</form> 
		                </td>
		            </tr>
		            	...
		            	<td colspan="10">
		            		<div style="padding:10px;">
		            			...
		            			<tr class="collapse in" style="background:#EBDEF0;">					
		            				...
		            				<?php
		            				$url = API_HOST_URL."get_all_transaction_sub_obligated.php?ledger_type_id=".$ledger_type_id."&b_year=".$fund_beginning_fiscal_year."&e_year=".$fund_ending_fiscal_year."&fund_id=".$fund_id."&pe_id=".$pe_id."&ledger_type=".str_replace(' ', '_', $ledger_type)."";
									$transaction_arr = requestByCURL($url);

									### get all transation for Obligated===============
									$url = API_HOST_URL."get_all_transaction_sub_obligated_to_account_payable.php?ledger_type_id=".$ledger_type_id."&b_year=".$fund_beginning_fiscal_year."&e_year=".$fund_ending_fiscal_year."&fund_id=".$fund_id."&pe_id=".$pe_id."&ledger_type=".str_replace(' ', '_', $ledger_type)."";
									...
									for($a=0; $a<count($transaction_arr['data']); $a++)
	            					{
	            						...
	            						{
	            							$credit_in = $transaction_arr['data'][$a]['ledger_type_id'].' ('.$transaction_arr['data'][$a]['ledger_type'].')'.' - Sub Obligated';
	            							$debit_from = $transaction_arr['data'][$a]['op_id'].' ('.$transaction_arr['data'][$a]['ledger_type_dr'].')'.' - Obligated';
	            						}
	            						else
	            						{
	            							$debit_from = $transaction_arr['data'][$a]['ledger_type_id'].' ('.$transaction_arr['data'][$a]['ledger_type'].')'.' - Sub Obligated';
	            							$credit_in = $transaction_arr['data'][$a]['op_id'].' ('.$transaction_arr['data'][$a]['ledger_type_dr'].')'.' - Obligated';
	            						} 
	            						?>
	            						<tr class="collapse in ">					
	            							<td><?php echo $debit_from; ?> </td>
	            							<td><?php echo $credit_in; ?> </td> 
	            							<td class="text_right"><?php if($transaction_arr['data'][$a]['credit_amount']>0)echo '<div title="SubObligated to  '.$credit_in.'"> $'.number_format($transaction_arr['data'][$a]['credit_amount']).'</div>';else echo '<div style="color:red;" title=" DeSubObligated to '.$credit_in.'"> - $'.number_format($transaction_arr['data'][$a]['debit_amount']).'</div>'; ?></td> 
            						...
            						?>
            						<tr class="collapse in ">
										<td><?php echo $reverse_from_arr['data'][$b]['ledger_type_id'].' ('.$reverse_from_arr['data'][$b]['ledger_type'].')'; ?> - Obligated</td> 
										<td><?php echo $reverse_from_arr['data'][$b]['op_id'].' ('.$reverse_from_arr['data'][$b]['op_type'].')';?> - Commited</td>
            							<td class="text_right"><?php echo '<div style="color:#E67E22;" title="DeObligated from '.$reverse_from_arr['data'][$b]['ledger_type_id'].' ('.$reverse_from_arr['data'][$b]['ledger_type'].')'.'"> $'.number_format($reverse_from_arr['data'][$b]['credit_amount']).'</div>'; ?></td> 
