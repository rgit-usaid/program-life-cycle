...
if(isset($_REQUEST['ledger_type_id']) and $_REQUEST['ledger_type_id']!='')
{
    ...
    $data = array();
    $select_fund_transaction = "select t.*,sum(td1.credit_amount) as total_un_alloted_amount
                        ...
                        where  td1.ledger_type='Program Element' and td1.credit_amount is not null and td1.ledger_type_id='".$ledger_type_id."' and t.fund_id='".$fund_id."' and td2.ledger_type='Operating Unit' and ou.type='Bureau' ".$cond."  group by td1.ledger_type_id";                                                
    ...
    {
        $data[$i]['total_un_alloted_amount'] = $fetch_fund_transaction['total_un_alloted_amount'];  
        $i++; 
    }
    if(count($data)>0){
