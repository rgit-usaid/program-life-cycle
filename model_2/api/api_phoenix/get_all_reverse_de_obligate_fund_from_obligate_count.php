...
if(isset($_REQUEST['ledger_type_id']) and $_REQUEST['ledger_type_id']!='')
{
    ...
    $data = array();
    $select_fund_transaction = "select t.*,sum(td1.credit_amount) as total_de_obligated_amount
                        ...
                        where  td1.ledger_type='Operating Unit' and td1.credit_amount is not null and td1.ledger_type_id='".$ledger_type_id."' and t.fund_id='".$fund_id."' and t.program_element_id='".$pe_id."' and td2.fund_status='Obligate' ".$cond."  group by td1.ledger_type_id";                                                
    ...
    {
        $data[$i]['total_de_obligated_amount'] = $fetch_fund_transaction['total_de_obligated_amount'];  
        $i++; 
    }
    if(count($data)>0){
