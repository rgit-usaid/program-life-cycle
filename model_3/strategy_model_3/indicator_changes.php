...
$error = '';
if(isset($_REQUEST['changes_save']))
{	
	$change_indicator = $mysqli->real_escape_string(trim($_REQUEST['change_indicator'])); 
	$other_note = $mysqli->real_escape_string(trim($_REQUEST['other_note'])); 
	
		$update_changes_data = "update usaid_custom_indicator set
			change_indicator = '".$change_indicator."',
			other_note = '".$other_note."'
			where id='".$custom_indicator_id."'";
		$result_data = $mysqli->query($update_changes_data); 
			if($result_data)
			{
						...
						<div class="container-fluid">
							...
							<div class="form-indicator">
								<form class="form-horizontal" role="form" method="post" action="">
									<label for="input-type-textarea">Changes to Indicator</label>
									<?php if($fetch_custom_indicator!='') $change_indicator= $fetch_custom_indicator['change_indicator']; ?>						
									<input id="input-type-text" name="change_indicator" value="<?php echo $change_indicator; ?>" type="text">

									<label for="input-type-textarea">Other Notes<span style="margin-left: 10px; font-size: 14px; color: #ccc; font-style: italic;">(Optional)</span></label>						<?php if($fetch_custom_indicator!='') $other_note= $fetch_custom_indicator['other_note']; ?>
									<textarea id="input-type-textarea" name="other_note"> <?php echo $other_note;  ?></textarea>

									<div class="button_wrapper clear">
										<button class="usa-button-outline" type="reset">Cancel</button>
										<button type="submit" name="changes_save" style="display:inline">Save & Exit</button>
									</div>
								</form>
							</div>
						</div>
