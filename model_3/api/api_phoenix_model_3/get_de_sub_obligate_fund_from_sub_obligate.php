...
if(isset($_REQUEST['ledger_type_id']) and $_REQUEST['ledger_type_id']!='')
{
    ...
    $select_fund_transaction = "select t.*,sum(td1.debit_amount) as total_de_obligated_amount
                        from usaid_fund_transaction as t
                        left join usaid_fund_transaction_detail as td1 ON td1.transaction_id = t.id
                        left join usaid_fund_transaction_detail as td2 ON td2.transaction_id = td1.transaction_id
                        where  td1.debit_amount is not null and td1.ledger_type_id in (".$ledger_type_id_str.") and t.fund_id='".$fund_id."' and t.program_element_id='".$pe_id."' and td1.fund_status='Subobligate' and td2.fund_status='Obligate' ".$cond."  group by td1.ledger_type_id";  
    $result_fund_transaction = $mysqli->query($select_fund_transaction) or die('Error'. $mysqli->error);
    $fetch_fund_transaction = $result_fund_transaction->fetch_array();
    $data['total_de_obligated_amount'] = $fetch_fund_transaction['total_de_obligated_amount']; // for commit in type id 
    if($data['total_de_obligated_amount']!=''){
