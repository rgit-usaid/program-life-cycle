...
{   
   $mydata = $str = str_replace('\\', '', $_REQUEST['x']);
   $mydata = unserialize(urldecode($mydata));
   print_r($mydata);
    //print_r(urldecode($mydata));
    ...
    $select_fund_transaction = "select td.transaction_year, sum(td.credit_amount) as credit_amount
                        ...
                        where td.ledger_type='Award CLIN' and td.fund_status='".$fund_status."' and td.credit_amount is not null and td.ledger_type_id='".$ledger_type_id."' group by td.ledger_type_id, td.transaction_year, td.fund_status";
