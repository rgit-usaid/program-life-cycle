...
function checkDateTimeCondition($frame_date_time, $other_date_time)
{
	$frame_date_time = strtotime($frame_date_time);
	$other_date_time = 	strtotime($other_date_time);
	if($frame_date_time > $other_date_time)
	{
		return true;
	}
	else return false;
...
{	
	...
	{	
		...
		if($fetch_res_frame >0)
		{
			...
			$_SESSION['frame_status']= $fetch_res_frame['status']; 
		}
	...
	$frame_status=$_SESSION['frame_status'];
...
function insertArchived($frame_id)
{
	global $mysqli;
	if($frame_id!='')
	{
		$select_frame = "select * from usaid_frame where id='".$frame_id."' and modified_on is not null";
		$res_frame = $mysqli->query($select_frame);
		$fetch_frame = $res_frame->fetch_array();
		$frame_date_time = $fetch_frame['modified_on'];
		if($fetch_frame['id']!='')
		{ 
			...
			if($archive_frame_id!='')
			{
				...
				{ 
					...
					{
						$goal_date_time = $fetch_goal_data['added_on'];
						if(checkDateTimeCondition($frame_date_time, $goal_date_time))
						{
							...
							goal_description = '".$mysqli->real_escape_string($fetch_goal_data['goal_description'])."',
				...
				if($total_DO_res->num_rows>0)
				{
					...
					{
						$do_date_time = $fetch_DO_data['added_on'];
						if(checkDateTimeCondition($frame_date_time, $do_date_time))
						{ 
							...
							objective_description = '".$mysqli->real_escape_string($fetch_DO_data['objective_description'])."',
				...
				if($total_IR_res->num_rows>0)
				{
					...
					{
						$ir_date_time = $fetch_IR_data['added_on'];
						if(checkDateTimeCondition($frame_date_time, $ir_date_time))
						{
							...
							ir_description = '".$mysqli->real_escape_string($fetch_IR_data['ir_description'])."',
				...
				if($total_Sub_IR_res->num_rows>0)
				{
					...
					{
						$sub_ir_date_time = $fetch_Sub_IR_data['added_on'];
						if(checkDateTimeCondition($frame_date_time, $sub_ir_date_time))
						{
							...
							sub_ir_description = '".$mysqli->real_escape_string($fetch_Sub_IR_data['sub_ir_description'])."',
...
## when save of any new Goal/DO/IR/SUB-IR  subsequently if Browser refresh (F5) button is clicked then the newly added boxes=============
function CheckBoxDescription($table, $whereVal)
{	global $mysqli;
	if($table && $whereVal!='')
	{
		$select_data = "select * from ".$table." ".$whereVal;
		$total_found_res = $mysqli->query($select_data); 
		$total_count_boxes = $total_found_res->num_rows;
		return $total_count_boxes;
	}	
...
## Add Development Goal =========================
	...
	{	
		...
		{	
			...
			if($frame_id!='')
			{
				$table="usaid_development_goal";
				$whereVal = "where goal_description='".removeDoubleQuote($goal_description)."' and frame_id='".$frame_id."'";
				$total_count_boxes = CheckBoxDescription($table, $whereVal);// calling for check any new Goal subsequently if Browser refresh (F5) 
				if($total_count_boxes<1)
				{	
					$insert_development_goal_data = "insert into usaid_development_goal set
					frame_id = '".$frame_id."',
					goal_description = '".removeDoubleQuote($goal_description)."',
					operating_unit_id = '".$operating_unit_id."',
					location = '".$location."',
	...
	{	
		...
		{	
			...
			if($frame_id!='')
			{
				$table="usaid_development_objective";
				$whereVal = "where objective_description='".removeDoubleQuote($objective_description)."' and frame_id='".$frame_id."'";
				$total_count_boxes = CheckBoxDescription($table, $whereVal);// calling for check any new DO subsequently if Browser refresh (F5) 
				if($total_count_boxes<1)
				{	
					$insert_development_objective = "insert into usaid_development_objective set
					frame_id = '".$frame_id."',
					objective_description = '".removeDoubleQuote($objective_description)."',
					operating_unit_id = '".$operating_unit_id."',
					location = '".$location."',
	...
	{	
		...
		{	
			...
			if($frame_id!='')
			{
				$table="usaid_intermediate_result";
				$whereVal = "where ir_description='".removeDoubleQuote($ir_description)."' and frame_id='".$frame_id."'";
				$total_count_boxes = CheckBoxDescription($table, $whereVal);// calling for check any new IR subsequently if Browser refresh (F5) 
				if($total_count_boxes<1)
				{	
					$insert_intermediate_result = "insert into usaid_intermediate_result set
					frame_id = '".$frame_id."',
					ir_description = '".removeDoubleQuote($ir_description)."',
					operating_unit_id = '".$operating_unit_id."',
					location = '".$location."',
	...
	{	
		...
		{	
			...
			if($frame_id!='')
			{
				$table="usaid_sub_intermediate_result";
				$whereVal = "where sub_ir_description='".removeDoubleQuote($sub_ir_description)."' and frame_id='".$frame_id."'";
				$total_count_boxes = CheckBoxDescription($table, $whereVal);// calling for check any new IR subsequently if Browser refresh (F5) 
				if($total_count_boxes<1)
				{	
					$insert_sub_intermediate_result = "insert into usaid_sub_intermediate_result set
					frame_id = '".$frame_id."',
					sub_ir_description = '".removeDoubleQuote($sub_ir_description)."',
					operating_unit_id = '".$operating_unit_id."',
					location = '".$location."',
...
{
	$frame_id = $_SESSION['frame_id'];
	insertArchived($frame_id); // function for insert archived which is return on top
	if($frame_id!='')
	{
		$updata_frame = "update usaid_frame set
		modified_on = NOW() 
				...
				if(data.length>0){ /*if association exists than fill the data else make popup blank*/
					$.each(data,function(index, assoc_obj){
						...
						{ 	 
							...
							if ($.inArray(project_id, flag_arr) < 0 || flag_arr.length==0)
							{
								flag_arr.push(project_id);
								$('#project_dpw').next('.ms-parent').find('input[type="checkbox"][value="'+project_id+'"]').trigger('click');
							}
							$('.assoc_with_act').trigger('click'); 
							 
							activity_arr[i] = assoc_obj.association_id;
							i++;
						}  
						
					});
					/*==loop for activity======*/
					for(var j=0; j<activity_arr.length; j++)
					{
						$('#project_act_dpw').next('.ms-parent').find('input[type="checkbox"][value="'+activity_arr[j]+'"]').trigger('click'); 
					}
	...
	<div class="container-fluid">
		<div class="header-detail">
			...
			<div class="row clear details">
				<div class="col-md-12"><a href="manage.php" class="remove-line"><?php echo $operating_unit_arr['data']['operating_unit_description']; ?></a> >> <a href="framework_management.php" class="remove-line"> Framework Management</a> >> <a href="framework_management.php" class="remove-line"><?php echo $frame_status; ?></a> >> <?php echo ucfirst($frame_name); ?></div>
			</div>
		</div>
	</div>
	<!-- Adding Adding Frame -->
	<div class="add-frame">
		<div class="container-fluid">
			<div class="col-md-9" style="text-align: center;">
					...
					<?php  ## Check for frame history ============= 
					$select_archives_frame="select * from usaid_archive_frame where frame_id = '".$frame_id."'";
					$result_archives_frame = $mysqli->query($select_archives_frame);
					$total_num_row = $result_archives_frame->num_rows; 
					?>
				<h2 class="text-center" style="margin-top: 30px;"><?php echo ucwords($frame_name); ?> (<?php echo $frame_status; ?>) <?php if($total_num_row>0) { ?> <a href="history_list_view.php?history_id=<?php echo $frame_id; ?>"> <span class="history-view">   View History</span></a> <?php }?> </h2>
			...
			<form id="form_assoc" method="post" action="" style="position:relative">
				<div id="myDiagramDiv" style="width:100%;min-height:600px;border:1px solid black;border-radius:10px;"></div>
				<!--popup-->
				<div id="popup">
					<!--popup close btn-->
					<div id ="popup_cover">
			...
			<div class="text-center">
				...
				<br>
				<span>“If screen appears blank OR relationship does not show after saving, please <a href="<?php echo $_SERVER['REQUEST_URI']; ?>"> Click Here</a>”</span>
			</div>
			</form>
		</div>
	</div>
...
<script>
	...
	$(document).ready(function () {
		...
		$(document).on('click','#project_dpw + .ms-parent input[type="checkbox"]', function(){
			...
			get_association_one_time(gohashid);		
		});
   ...
   $('#save_assoc').click(function(){
	...
	/*used to when check or uncheck project==========*/
		function get_association_one_time(gohashid)
		{
			var prj_act_elem = $('#project_act_dpw').next('.ms-parent').find('input[type="checkbox"]');
			//alert(prj_act_elem.length);
			jQuery.ajax({
				type:'POST',
				url:'ajaxfiles/get_association.php',
				data:{gohashid:gohashid},
				success:function(data){
					var data = JSON.parse(data); 
					var i=0;
					var activity_arr1 = new Array(); 
					if(data.length>0){ /*if association exists than fill the data else make popup blank*/
						$.each(data,function(index, assoc_obj){ 
							if(assoc_obj.association_type=="Activity")
							{  
								activity_arr1[i] = assoc_obj.association_id;
								i++;
							}  
					...
					/*==loop for activity selection======*/ 
					for(var p=0; p<activity_arr1.length; p++)
					{
						if($.inArray(activity_arr1[p],prj_act_elem))
						{
							$('#project_act_dpw').next('.ms-parent').find('input[type="checkbox"][value="'+activity_arr1[p]+'"]').trigger('click');
						}	
			...
			var prj_elem = $('#project_dpw').next('.ms-parent').find('input[type="checkbox"]:checked');
			if(prj_elem.length==0)
			{
				$('#project_act_label_blk').find('.disp_text').html("");
			}
		 } 
// -------------new code to remove to activity and project on tags click--------------
$(document).on('click',".unchk", function(){
	console.log("span clicked");
	var cls = $(this).parent('span').attr('class').split("-")[1];
	// var cls2 = $(this).parent('span').attr('class').split("-", 2);
	var cls2 = $(this).parent('span').attr('class').split(/-(.+)/)[1];
	$(this).closest('.disp_text').prev('.ms-parent').find(".ms-drop").find('input[type="checkbox"]:checked[value='+cls+']').trigger('click');
	$(this).closest('.disp_text').prev('.ms-parent').find(".ms-drop").find('input[type="checkbox"]:checked[value='+cls2+']').trigger('click');
