...
if(isset($_REQUEST['ledger_type_id']) and $_REQUEST['ledger_type_id']!='')
{
    ...
    $select_fund_transaction = "select t.*,f.fund_name,f.fund_id as fund_code,ou.operating_unit_abbreviation,ou.operating_unit_description,td1.ledger_type_id ,sum(td1.credit_amount) as total_amount,ou1.operating_unit_abbreviation as origination_point,td2.ledger_type_id as op_id
                            ...
                            where td1.ledger_type='Operating Unit' and td2.ledger_type='Operating Unit' and ou.type!='Bureau' and ou1.type='Bureau' and td1.credit_amount is not null and td1.ledger_type_id='".$ledger_type_id."' and t.fund_id='".$fund_id."'and t.program_element_id='".$pe_id."' ".$cond." group by td2.ledger_type_id,t.fund_beginning_fiscal_year,t.fund_ending_fiscal_year";
