...
function insertArchiveGeoLocationData($project_id)
{
	global $mysqli;
	if($project_id!='')
	{
...
if(isset($_REQUEST['add_location']) && isset($_REQUEST['project_id']) && $_REQUEST['project_id']!="")
{
	...
	if($_REQUEST['geo_location_id']==""){
		insertArchiveGeoLocationData($project_id); // call for insert archive geo location 
			...
			<div class="project-detail-blk table-container">
				...
				<div class="form-blk">
					<div class="row">
						...
						<div class="col-lg-8 col-md-8 col-sm-8 col-xs-12 loc_indicator_marker disp-none">
					 ...
					 <div class="row">
						<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
							  ...
							  <div class="row">
								 <div class="col-lg-5 col-md-5 col-sm-5 col-xs-12">
									 ...
									 <div class="usa-search">
										 <div role="search">
										  ...
										  <input id="txtPlaces" type="search" name="search" class="wdth" palceholder="Enter a Location">
										  <button type="submit" id="add_google">
											<span class="usa-search-submit-text" >Search</span>
										  </button>
										 </div>
									 </div>
								 </div>
								 <div class="col-lg-2 col-md-2 col-sm-2 col-xs-12 text-center" style="font-size:24px;color:#ff5500">
								 ...
								 <div class="col-lg-5 col-md-5 col-sm-5 col-xs-12">
									<div>
										...
										<div style="font-size:12px; font-style:italic; margin-bottom:5px" class="text-danger">Search eg: 40.714224,-73.961452</div>
									</div>
									<div class="extra_ht"></div>	
									<div class="usa-search">
										<div id="floating-panel" role="search">
										  <label class="usa-sr-only" for="search-field">Search medium</label>
										  <input id="latlng" type="search" value="" class="wdth"  placeholder="Enter Lat-Lng" style="max-width:250px">
										  <button type="submit" id="submit" style="width:150px">
											<span class="usa-search-submit-text" >Reverse Geocode</span>
										  </button>
										</div>
									</div>
								   </div>
							...
							<!-- <div class="input-group">
								<input type="text" class="form-control"   placeholder="Enter a location" aria-describedby="basic-addon2">
								<span class="input-group-addon btn btn-blue" >Search</span>
							  </div>-->
							<div class="extra_ht"></div><div class="extra_ht"></div>
						</div>
					 </div>
					 <div class="gray-line"></div>
					 <form id="add_geo_loc" method="post" autocomplete="off">
						...
						<div class="row">
					 	<div class="col-lg-4 col-md-3 col-sm-6 col-xs-12">
							...
							<div>
								<textarea id="geo_address" name="address"><?php if(isset($edit_mode)) { echo $address;}?></textarea>
							</div>
						</div>
						<div class="col-lg-4 col-md-3 col-sm-6 col-xs-12">
							...
							<div>
								<textarea  id="geo_latitude" name="latitude"><?php if(isset($edit_mode)) { echo $latitude;}?></textarea>
							</div>
						</div>
						<div class="col-lg-4 col-md-3 col-sm-6 col-xs-12">
							...
							<div>Coordinates</div><br/>
							<div>
								<textarea  id="geo_longitude" name="longitude"><?php if(isset($edit_mode)) { echo $longitude;}?></textarea>
							</div>
						</div>
						<div class="col-lg-4 col-md-3 col-sm-6 col-xs-12">
							 <header><h3>Precision</h3></header>
							 <input type="text" name="precision_code" value="<?php if(isset($edit_mode)) { echo $precision_code;}?>"/>
						</div>
						<div class="col-lg-4 col-md-3 col-sm-6 col-xs-12">
							 <header><h3>Location Type</h3></header>
							 <input type="text" name="location_type" value="<?php if(isset($edit_mode)) { echo $location_type;}?>"/>
						</div>
						<div class="col-lg-4 col-md-3 col-sm-6 col-xs-12">
							 <header><h3>Country</h3></header>
							 <input type="text" name="country" value="<?php if(isset($edit_mode)) { echo $country;}?>"/>
						</div>
						<div class="col-lg-4 col-md-3 col-sm-6 col-xs-12">
							 <header><h3>Activity</h3></header>
							<div>
								<select name="project_activity_id" class="form-control">
						...
						<div class="col-lg-4 col-md-3 col-sm-6 col-xs-12">
							 <header style="padding-top:5px"><h3><input type="checkbox" name="centrally_managed" value="Yes" <?php if(isset($edit_mode) && $centrally_managed=="Confirmed") {?> checked="checked" <?php }?> id="centrally_managed"/><label style="font-size:20px">Centrally Managed</label></h3></header>
							 
						</div>
						<div class="col-lg-4 col-md-3 col-sm-6 col-xs-12">
							 <header style="padding-top:5px"><h3><input type="radio" name="impacted_area" value="No" <?php if(isset($edit_mode) && $impacted_area=="No") {?> checked="checked" <?php }?>/><label style="font-size:20px">Implementation Area</label></h3></header>
							 <header style="padding-top:5px"><h3><input type="radio" name="impacted_area" value="Yes" <?php if(isset($edit_mode) && $impacted_area=="Yes") {?> checked="checked" <?php }?>/><label style="font-size:20px">Area of Impact</label></h3></header>
						</div>
					 </div>
					 	<div class="row">
							<div class="col-lg-4 col-md-3 col-sm-6 col-xs-12">
								...
								<button class="usa-button-outline" type="button" id="cancel_geo_loc">Cancel</button>
								<button class="usa-button-active" id="save_geo_loc" type="button">Save</button>
...
<script type="text/javascript">
	...
	function geocodeLatLng(geocoder, map, infowindow) {
		...
		geocoder.geocode({'location': latlng}, function(results, status) {
			var newLoc = new Array(results[1].formatted_address,lat,lng,4,"<?php echo SITE_PATH;?>usaid/amp/2/img/flag.png");
			...
			var marker = new google.maps.Marker({
				...
				map: map,
				zoom: 10,
				icon: "<?php echo SITE_PATH;?>usaid/amp/2/img/flag.png",
		 	});
			$('#latlng').val("");
	...
	google.maps.event.addDomListener(window, 'load', function () {
		...
		google.maps.event.addListener(places, 'place_changed', function () {
			...
			/*for(places in result) {
				for(result.address_components as $addressPart) {
	...
	$('#add_google').click(function(){
		...
		if(curr_geo_loc["address"]!="" && curr_geo_loc["latitude"]!="" && curr_geo_loc["longitude"]!=""){
			...
			var newLoc = new Array(tooltip_add,curr_geo_loc["latitude"],curr_geo_loc["longitude"],4,"<?php echo SITE_PATH;?>usaid/amp/2/img/flag.png");
			dummy_search_locations.push(newLoc);
	...
	$('#save_geo_loc').click(function(){
		...
		if(error=="error"){
			$('#add_geo_loc').find('.form-msg').addClass('usa-alert-error').removeClass('disp-none');
			$('#add_geo_loc').find('.form-msg').find('.usa-alert-heading').html('Error');
			$('#add_geo_loc').find('.form-msg').find('.usa-alert-text').text(error_msg);
			setTimeout(function(){
				$('#add_geo_loc').find('.form-msg').addClass('disp-none');
			},5000);	
		}
	});
