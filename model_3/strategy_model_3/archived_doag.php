...
$select_object_agreement="select * from usaid_objective_agreement where operating_unit_id='".$operating_unit_id."' and status='Archive'";
...
<body>
	...
	<div class="container-fluid">
		<div class="header-detail">
			...
			<div class="row clear details">
				<div class="col-md-12"><a href="manage.php" class="remove-line"><?php echo $operating_unit_arr['data']['operating_unit_description']; ?></a> >> <a href="go_to_doag.php" class="remove-line"> DOAGs and SOAGs</a> >> Archived >> List</div>
			</div>
		</div>
	</div>
	...
	<div class="container-fluid">
		<table id="manage-table" class="table table-striped table-bordered" cellspacing="0" width="100%">
			<thead>
				<tr>
					<td colspan="8" style="background:white;">Archived List of DOAGs and SOAGs </td>
				</tr>
				<tr>
			...
			while($fetch_object_agreement = $result_object_agreement->fetch_array())
			{
			 ?>
				<tr class="parent-link" height="5" style="padding:3px; !important">
					...
					<td><?php echo dateFormatView($fetch_object_agreement['modified_on']);?></td>
					<td valign="top">
	...
	$total_num_row = $result_archives_objective_agreement->num_rows; if($total_num_row>0) {?>
					<a href="doag_history_list.php?history_id=<?php echo $fetch_document['id']; ?>">View History </a> <?php } else echo "No History"; ?></td>
					 
					<td class="text-center">
						<form method="get" action="view_archived_doag.php" style="display:inline">
						<input type="hidden" name="objective_agreement_view_id" value="<?php  echo $fetch_object_agreement['id']; ?>">
						<button name="edit"  class="btn btn-warning"><i class="fa fa-eye" aria-hidden="true"></i></button>
						</form>
					</td>								
				</tr>
