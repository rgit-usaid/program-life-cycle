		...
		<div class="data-display container-fluid  table-responsive"> 
	        ...
	        if(count($unique_fund_arr['data'])!=NULL)
	        {
	        	for($i=0; $i<count($unique_fund_arr['data']); $i++)
			    {
					...
					### get total obligated direct by OU=============
				    ...
				    ### get total obligated fund for commit==========
					$url = API_HOST_URL."get_total_obligated_for_report_by_ou.php?ledger_type_id=".$ledger_type_id."&b_year=".$fund_beginning_fiscal_year."&e_year=".$fund_ending_fiscal_year."&fund_id=".$fund_id."&pe_id=".$pe_id."";
					$obli_balance_arr = requestByCURL($url);
					$total_obli_amount = $obli_balance_arr['data']['total_obliagted']; 

				    ### get total reverse de-obligated  fund from obligate screen to operating unit==========
					...
					$total_de_obligated_amount = $de_obligated_balance_arr['data'][0]['total_de_obligated_amount'];
					 ...
					 ### get total reverse de-obligated  fund from obligate screen to Commit==========
					$url = API_HOST_URL."get_total_deobligated_from_obligate_to_commit.php?ledger_type_id=".$ledger_type_id."&b_year=".$fund_beginning_fiscal_year."&e_year=".$fund_ending_fiscal_year."&fund_id=".$fund_id."&pe_id=".$pe_id."";
					$de_obligated_balance_arr = requestByCURL($url);
					$total_de_obligated_to_commit = $de_obligated_balance_arr['data']['total_de_obliagted'];
					
					### actual spent fund for this operating unit==========
					$url = API_HOST_URL."get_doag_award_by_ou.php?ledger_type_id=".$ledger_type_id."&b_year=".$fund_beginning_fiscal_year."&e_year=".$fund_ending_fiscal_year."&fund_id=".$fund_id."&pe_id=".$pe_id."";
					$sub_obligate_arr = requestByCURL($url);
					 
					### de sub obligated fund from sub obligated amount==========
					$url = API_HOST_URL."get_de_sub_obligate_fund_from_sub_obligate.php?ledger_type_id=".$ledger_type_id."&b_year=".$fund_beginning_fiscal_year."&e_year=".$fund_ending_fiscal_year."&fund_id=".$fund_id."&pe_id=".$pe_id."";
					$de_sub_obligate_arr = requestByCURL($url);

					$net_sub_obligated_fund = $sub_obligate_arr['data']['total_sub_obliagted'] - $de_sub_obligate_arr['data']['total_de_obligated_amount'];
					### variable for net obligated from ou====
					$net_obligated_from_ou = $total_obligated_amount - $total_de_obligated_amount;
  
					### actual spent fund for this operating unit==========
					$url = API_HOST_URL."get_total_paid_for_report_by_ou.php?ledger_type_id=".$ledger_type_id."&b_year=".$fund_beginning_fiscal_year."&e_year=".$fund_ending_fiscal_year."&fund_id=".$fund_id."&pe_id=".$pe_id."";
					$paid_arr = requestByCURL($url);
					$total_paid = $paid_arr['data']['total_paid_amount'];

					##total obligated to commit======
					$net_obligated_to_commit = $total_obli_amount - $total_de_obligated_to_commit; 
			?>	
				<tr>
					<td colspan="2">
						<div class="">
						  	<div class="divide" >
								<div class="row" >
									...
									<div class="col-sm-6 text-center"><h4> Received: <?php echo '$'.number_format($net_committed_amount+$net_obligated_from_ou+$closing_balance_arr['closing_balance']); ?></h4></div> 
							 	</div>
						 	</div> 
							<table class="table table-striped table-bordered clearfix table1" cellspacing= "0" width="100%">
								<thead>
								...
								<tbody> 
									<tr> 
										...
										<td><?php echo '$'.number_format($net_obligated_from_ou);?></td> 
