		...
		if($exe_review->num_rows<=0){
			...
			if(!$upd){
				...
				if(count($output_ids)==0){
					...
					foreach($project_output_score_description as $key=> $val){
						##===new output is added
						$i=$key;
						if($output_ids[$i]!="" && $project_output_score_description[$i]!="" && $project_output_impact_weight[$i]!="" && $project_output_performance[$i]!="" && $project_output_risk[$i]!="" && $edit_row[$i]=="edit_row"){
							$project_output_score_description[$i] = $mysqli->real_escape_string($project_output_score_description[$i]);
							$upd_opt_score = "UPDATE usaid_project_monitoring_output_score set project_output_score_description='".$project_output_score_description[$i]."', project_output_impact_weight='".$project_output_impact_weight[$i]."', project_output_performance='".$project_output_performance[$i]."', project_output_risk='".$project_output_risk[$i]."' WHERE id='".$output_ids[$i]."'";
							
							$exe_opt_score = $mysqli->query($upd_opt_score);
						}
						else if($output_ids[$i]==""){
							$project_output_score_description[$i] = $mysqli->real_escape_string($project_output_score_description[$i]);
	...
	<div class="main-container container-fluid" id="main-content">
		...
		<div class="tbl-block">
			<div class="tbl-caption">
				<div class="tbl-content-head"><?php if(!isset($edit_mode)){?>Add New Project Review<?php } else {?>Edit Review<?php }?></div>
			...
			<div class="project-detail-blk table-container">
				...
				<div class="form-blk">
					...
					<div class="row">
						<div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
							<div class="threshold">
								...
								<label>
									<input type="radio" name="annual_review_required" value="Y" class="review_req" <?php if($ar_review_exits){?> checked="checked" <?php } else {?> disabled="disabled" <?php }?>/> Yes
								</label>
								<br/>
								<label>
									<input type="radio" name="annual_review_required" value="N" <?php  if(!$ar_review_exits){?> checked="checked" <?php } else {?> disabled="disabled" <?php }?> class="review_req"/> No
								</label>
					...
					<div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
						<div class="threshold">
							...
							<label>
								<input type="radio" name="project_activity_review_required" value="Y" <?php if($par_review_exits){?> checked="checked" <?php } else {?> disabled="disabled" <?php }?> class="review_req"/> Yes
							</label>
							<br/>
							<label>
								<input type="radio" name="project_activity_review_required" value="N" <?php if(!$par_review_exits){?> checked="checked" <?php } else {?> disabled="disabled" <?php } ?> class="review_req"/> No
							</label>
						</div>
					</div>
				...
				<div class="form-blk">
					...
					<a class="btn btn-blue <?php if(isset($edit_mode)){?>disp-none<?php }?>" id="add_new_review"><i class="fa fa-plus-circle" aria-hidden="true"></i> Add Review</a>
					<!--add review block start-->
					<form id="add_review_form" method="post" autocomplete="off">
									...
									<div class="row">
										...
										<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
											<div class="review_doc_rblk_out">
												...
												<textarea class="form-control autoh_textarea" name="annual_review_approver_comments" rows="2"><?php if(isset($edit_mode)){ echo $annual_review_approver_comments; }?></textarea>
												<div class="extra_ht"></div>
												<div class="threshold">
													<label>
														<input type="radio" name="annual_review_approval" value="Approve" class="review_req project_status" <?php if(isset($edit_mode) && ($annual_review_approval=="Approve")){ echo "checked='checked'"; }?>/> Approve
													</label>
													<label style="margin-left:20px">
														<input type="radio" name="annual_review_approval" value="Reject" class="review_req project_status" <?php if(isset($edit_mode) && ($annual_review_approval=="Reject")){ echo "checked='checked'"; }?> <?php if(isset($edit_mode) && ($annual_review_approval=="Approve")){?> disabled="disabled"  <?php }?>/> Reject
													</label>
						...
						<div class="row review_ip_blk disp-none">
							<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
								...
								<input type="button" name="cancel_review" class="btn btn-blue" value="Cancel" id="cancel_review"/>
								<input type="button" name="save_review" class="btn btn-green <?php if(isset($edit_mode) && $annual_review_approval=="Approve"){?> disp-none <?php }?>" value="Save Review" id="save_review"/>	
								<div class="form-msg disp-inline"></div>	
							</div>
						</div>
					</div>
					</form>
...
<script>
	...
	$('#add_output_score').click(function(){
		...
		var html= '<tr class="output_score_row"><td><input type="hidden" value="" name="output_id[]" /><textarea class="form-control autoh_textarea" name="project_output_score_description[]" rows="2"></textarea></td><td><input type="text" class="form-control impact_perc only_num"  name="project_output_impact_weight[]"/></td><td>'+performance_dpw+'</td><td>'+risk_dpw+'</td><td class="text-center"><a class="btn btn-danger remove_output_score" title="Delete Review"><i class="fa fa-trash-o" aria-hidden="true"></i> </a></td></tr>';
	...
	$('#save_review').click(function(){
		...
		*/
		...
		if(error=="error"){
			$('#add_review_form').find('.form-msg').addClass('error');
			$('#add_review_form').find('.form-msg').html(error_msg);
			setTimeout(function(){
				$('#add_review_form').find('.form-msg').html("");
			},5000);	
		}
	});
