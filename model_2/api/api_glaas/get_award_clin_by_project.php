...
## get all award clin of a project ============

if(isset($_REQUEST['project_id']) and $_REQUEST['project_id']!='')
{   
    $data = array();
    $project_id = trim($_REQUEST['project_id']);
	$select_data = "select rac.id as clin_id, ra.vendor_id, rac.award_number, rac.clin_number, rac.clin_name, rac.clin_description, rac.clin_amount as amount, rcb.*, v.id as vendorid, v.DUNS_number, v.name
					...
					where al.link_to_id='".$project_id."' and al.link_to_type='Project' and al.associate_from_type='Clin' group by rcb.budget_number";
    ...
    while($fetch_data = $result_data->fetch_array())
    {
		 ...
		 ## get all usid_requisition_award_clin by  project ============
