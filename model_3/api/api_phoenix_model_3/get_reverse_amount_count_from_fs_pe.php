...
if(isset($_REQUEST['ledger_type_id']) and $_REQUEST['ledger_type_id']!='')
{
    ...
    $data = array();
    $select_fund_transaction = "select t.*,sum(td1.credit_amount) as total_fs_pe_rev_amount
                        ...
                        where  td1.ledger_type='Fund' and td1.credit_amount is not null and td1.ledger_type_id='".$ledger_type_id."' and t.fund_id='".$ledger_type_id."' and td2.ledger_type='Program Element' ".$cond."  group by td1.ledger_type_id";                                                
    ...
    {
        $data[$i]['total_fs_pe_rev_amount'] = $fetch_fund_transaction['total_fs_pe_rev_amount'];  
        $i++; 
    }
    if(count($data)>0){
