...
if(isset($_REQUEST['ledger_type_id']) and $_REQUEST['ledger_type_id']!='')
{
   ...
   $select_fund_transaction = "select t.*, sum(td1.credit_amount) as total_commited_amount
                        from usaid_fund_transaction as t
                        left join usaid_fund_transaction_detail as td1 ON td1.transaction_id = t.id
                        where td1.ledger_type='Project Activity' and td1.fund_status='Commit' and td1.credit_amount is not null and td1.ledger_type_id='".$ledger_type_id."' ".$cond."  group by td1.ledger_type_id, t.fund_beginning_fiscal_year, t.fund_ending_fiscal_year";
    ...
    while($fetch_fund_transaction = $result_fund_transaction->fetch_array())
    {
         ...
         $data['total_commited_amount'] = $fetch_fund_transaction['total_commited_amount'];
