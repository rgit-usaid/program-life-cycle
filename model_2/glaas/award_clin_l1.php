...
unset($_SESSION['archive_clin_l1_id']);
...
##generate CLIN number by award=========
function generateClinNumber($award_number)
{
	$url = API_HOST_URL."get_all_award_clin_l1_by_award.php?flag=1&award_number=".$award_number;
	$award_arr = requestByCURL($url);
	$total_count = count($award_arr['data']);
	$total_count  = $total_count + 1;
	$last_number = sprintf("%03d",$total_count);
	$clin_number = $award_number.'-'.$last_number;
	return $clin_number;
}
###get requisition number or award number from request=========
$requisition_number = '';
$award_number = '';
$level = 1;
if(isset($_REQUEST['award_number_clin']))
{
	$award_number = trim($_REQUEST['award_number_clin']);
	$_SESSION['award_number'] = $award_number;
	$requisition_number = trim($_REQUEST['award_requisition_number']);
...
if(isset($_REQUEST['save_clin_l1']))
{
	...
	if(!isset($_REQUEST['do_not_share']))$do_not_share = 'N';
	else $do_not_share = $_REQUEST['do_not_share'];
	...
	else
	{
		if($clin_id=='')
		{
			...
			header("location:award_clin_l1.php");
		}
		else
		{
...
### get header details or clin level 1 details for listing=======
if($award_number!='')
{
	...
	$clin_number = generateClinNumber($award_number);	
}

## edit clin level 1 =================
$flag = 0;
if(isset($_REQUEST['edit_clin_l1']))
{
	...
	## get all assoc link for award clin level 1 =======	
	$url = API_HOST_URL."get_link_to_by_requisition.php?requisition_number=".$edit_clin_number;
	$assoc_link_arr = requestByCURL($url);
}

## delete_clin_l1 =================
if(isset($_REQUEST['delete_clin_l1']))
{
	$edit_clin_number = $_REQUEST['edit_clin_number'];
	removeAwardClin_l1($edit_clin_number);
	header("location:award_clin_l1.php");  
}
?>
		...
		<div class="container-fluid">
			<table id="manage-parent-table" class="table table-striped" border="2" style="" cellspacing="0" width="100%">
				...
				<tbody>
					...
					{
						for($k=0; $k<count($award_clin_l1_arr['data']); $k++)
						{
							$url = API_HOST_URL."get_all_budget_of_clin.php?clin_number=".$award_clin_l1_arr['data'][$k]['clin_number'];
							$clin_budget_arr = requestByCURL($url);
							$url = API_HOST_URL."get_link_to_by_requisition.php?requisition_number=".$award_clin_l1_arr['data'][$k]['clin_number'];
							$list_assoc_link_arr = requestByCURL($url); 
							?>	
						<tr class="parent-tbl parent-link">
							<td class="text-center"><span onClick="showClinTwo(this)" class="btn action" title="Level-1"><img src="images/plus.png"></span> <?php echo $award_clin_l1_arr['data'][$k]['clin_number']; ?></td>
							<td class="text-center"><?php echo $award_clin_l1_arr['data'][$k]['clin_name']; ?></td>
							<td class="text-center"><?php echo $award_clin_l1_arr['data'][$k]['clin_description']; ?></td>
							<td class="text-right"><?php echo '$'.number_format($award_clin_l1_arr['data'][$k]['clin_amount']); ?></td>
							<td class="text-center"><?php echo $award_clin_l1_arr['data'][$k]['start_performance_period']; ?></td>
							<td class="text-center"><?php echo $award_clin_l1_arr['data'][$k]['end_performance_period']; ?></td>
									...
									<ul class="list-inline" style="">
										<li>
											<form method="post" action="">
												<input type="hidden" name="edit_clin_number" value="<?php echo $award_clin_l1_arr['data'][$k]['clin_number'];?>">
												<button type="submit" name="edit_clin_l1" class="btn btn-success action" data-toggle="tooltip" data-placement="top" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i></button>
												<button type="submit" name="delete_clin_l1" class="btn btn-danger action" data-toggle="tooltip" data-placement="top" title="Remove" onClick="return window.confirm('Are you sure you want to remove this?');"><i class="fa fa-times" aria-hidden="true"></i></button>
										...
										<li>
											<form method="post" action="award_clin_l2.php">
												<input type="hidden" name="clin_l1_number" value="<?php echo $award_clin_l1_arr['data'][$k]['clin_number'];?>">
												...
												<button type="submit" class="btn btn-default action" data-toggle="tooltip" data-placement="top" title="AWARD CLIN - L2" style="margin:0 2px; color:#000; font-size: 12px; text-decoration:none"> AWARD CLIN - L2 </button>
									<?php 
								 	 $url = API_HOST_URL."get_all_archive_award_clin_detail.php?archive_clin_id=".$award_clin_l1_arr['data'][$k]['clin_id'];
									 $archive_award_req_arr = requestByCURL($url); if(count($archive_award_req_arr['data'])>0) { ?>
									<a href="award_clin_l1_archive_list.php?archive_clin_l1_id=<?php echo $award_clin_l1_arr['data'][$k]['clin_id'];?>">Change Log</a>
								...
								<div style="padding:10px;">
									<table id="manage-parent-table" class="table table-striped"  cellspacing="0" width="100%" border="1">
										...
										## get all clin_l2 of this requisition
											$url = API_HOST_URL."get_all_award_clin_l2_by_clin1.php?clin_l1_number=".$award_clin_l1_arr['data'][$k]['clin_number'];
	...
	<div class="container-fluid" id="form-requisiton">
				<div class="row">
					<form class="form-horizontal" role="form" method="post" action="">			
						<input type="hidden" name="clin_id" value="<?php echo $clin_l1_arr['data']['clin_id'];?>">	
						<div class="col-md-6">
							<div class="container-fluid" id="manage-vendor">
								<div class="form-title" style="width:200px;">
									<i class="fa" aria-hidden="true"></i>AWARD CLIN - L1
								</div>
									...
									<div class="manage-info"> 
										...
										<div class="form-group">
											...
											<div class="col-md-6">								
												...
												if(isset($clin_l1_arr['data'])){$clin_name = $clin_l1_arr['data']['clin_name'];}
												?>		
										...
										<div class="form-group">
											...
											<div class="col-md-6">								
												...
												if(isset($clin_l1_arr['data'])){$clin_description = $clin_l1_arr['data']['clin_description'];}
												?>								
										...
										<div class="form-group">
											...
											<div class="col-md-6">							
												...
												if(isset($clin_l1_arr['data'])){$clin_amount = '$'.number_format($clin_l1_arr['data']['clin_amount']);}
												?>	
										...
										<div class="form-group">
											...
											<div class="col-md-6">
												<div class="input-group">
													...
													if(isset($clin_l1_arr['data'])){$start_performance_period = $clin_l1_arr['data']['start_performance_period'];}
													?>								
										...
										<div class="form-group">
											...
											<div class="col-md-6">
												<div class="input-group">
													...
													if(isset($clin_l1_arr['data'])){$end_performance_period = $clin_l1_arr['data']['end_performance_period'];}
													?>
										...
										<div class="form-group">
											...
											<div class="col-md-6">							
												...
												if(isset($clin_l1_arr['data'])){$modification_number = $clin_l1_arr['data']['modification_number'];}
												?>	
										...
										<div class="form-group">
											...
											<div class="col-md-6">							
												...
												if(isset($clin_l1_arr['data'])){$modification_purpose = $clin_l1_arr['data']['modification_purpose'];}
												?>	
										...
										<div class="form-group">
											...
											<div class="col-md-6">								
												...
												if(isset($clin_l1_arr['data'])){$operating_unit_id = $clin_l1_arr['data']['operating_unit_id'];}
										...
										<div class="form-group">
											...
											<div class="col-md-6">								
												...
												if(isset($clin_l1_arr['data'])){$employee_id = $clin_l1_arr['data']['employee_id'];}
										...
										<div class="form-group">
											...
											<div class="col-md-6">								
												...
												if(isset($clin_l1_arr['data'])){$do_not_share = $clin_l1_arr['data']['do_not_share'];}
												?>
							  ...
							  <?php } ?>					
										...
										<div class="form-group">
											<div class="col-md-6">
												<h5 class="pull-right"><a href="award_instrument.php">Back to Award</a></h5>
											</div>
											<div class="col-md-6">
												<a href="" class="btn btn-default" style="margin-right:25px;">Cancel</a>
												<button type="submit" class="btn btn-primary save" name="save_clin_l1" >Save</button>
											</div>
										</div> 
									</div>
								</div>
								...
								<tbody id="append_Budget">					
									...
									<tr class="Budget-info">						
										...
										<td>
											<select class="form-control" name="code_description[]" onChange="getCodeDescription(this)"; >
												<option value="">Select</option>
												<option value="Supplies=2001">Supplies (2001)</option>
												<option value="Travel and Per diem=3001">Travel and Per diem (3001)</option>
												<option value="Benefits=1002">Benefits (1002)</option>
												<option value="Equipment=2002">Equipment (2002)</option>
												<option value="Leases and Rentals=4001">Leases and Rentals (4001)</option>
												<option value="Consultants=1003">Consultants (1003)</option>
												<option value="Operating expenses=2003">Operating expenses (2003)</option>
												<option value="Other expenses=5001">Other expenses (5001)</option>
											</select>
										</td>
										<td>
