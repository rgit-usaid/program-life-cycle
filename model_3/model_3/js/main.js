...
$.fn.validate_ip = function() {  
	...
	if(this.hasClass('only_num')){ 
		...
		if(val.search(/^[0-9]+$/)==-1 && $(this).val()!=""){
			$(this).closest('.validate_ip_par').addClass('usa-input-error');
			$(this).addClass('error_ip invalid_ip');	
		}
		else{
			$(this).closest('.validate_ip_par').removeClass('usa-input-error');
			$(this).removeClass('error_ip invalid_ip');
		}
	}
	else if(this.hasClass('money_format')){ 
		if(val!=""){
			if(val.search(/^[0-9$,]+$/)!=-1){
...
$(document).on('click','body',function(e){
	if(!(e.target.id=="find_all_project" || e.target.className.search('elem')==0)){
	   $('.ajax_data').html("");
	}
});
