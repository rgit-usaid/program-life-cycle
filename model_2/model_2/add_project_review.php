			...
			if(!$upd){
				...
				if(count($output_ids)==0){
					...
					foreach($output_ids as $key=> $val)
					{
						...
						{
							$project_output_score_description[$i] = $mysqli->real_escape_string($project_output_score_description[$i]);
							$upd_opt_score = "UPDATE usaid_project_monitoring_output_score set project_output_score_description='".$project_output_score_description[$i]."', project_output_impact_weight='".$project_output_impact_weight[$i]."', project_output_performance='".$project_output_performance[$i]."', project_output_risk='".$project_output_risk[$i]."' WHERE id='".$val."'";
								...
								<table class="table table-bordered review-listing table-striped">
									...
									<tr class="data_tr">
										...
										<td class="text-center"><?php  
											...
											<a href="view_project_review_archive.php?project_monitoring_id=<?php echo $project_review_arr['data'][$i]['review_id'];?>">Change Log</a><?php } else { ?><a href="javascript:void(0)">No Change Log</a><?php }?></td>
										<td class="text-center">
