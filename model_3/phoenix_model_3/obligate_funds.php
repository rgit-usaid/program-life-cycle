...
if(isset($_REQUEST['obligate_fund']))
{
	...
	$ledger_type_cr = trim($_REQUEST['obligate_type']); //like ledger type cr
	$ledger_type_id_cr = trim($_REQUEST['commit_in_id']); //like ledger type id cr
		...
		<div class="new-form-content">
			...
			<div class="manage-funds"> 
				<form class="form-horizontal" role="form" method="post" action="">  
					...
					<div class="form-group">
						<label class="col-md-4" for="Commit To">Obligate To:</label>
						<div class="col-md-8">
							<select class="form-control commit_type" name="obligate_type" onchange="getInputFieldByCommitType();">
								<option value="">Select</option>
	...
	<div style="float:right;margin-bottom:5px;">
			 <table >
			 	<tr>
					...
					<td style="background:#2E9AFE;width:20px;">&nbsp;</td><td>&nbsp; SubObligated/Paid&nbsp;&nbsp;</td>
			 		<td style="background:#E67E22;width:20px;">&nbsp;</td><td>&nbsp; De-SubObligated</td>
			 	</tr>
			 </table>
		</div> 
		<table id="manage-table" class="table table-striped table-bordered" cellspacing="0" width="100%">
	        ...
	        { 
	            ...
	            { 
					...
					$unobligated_balance = $unobligated_balance_arr['total_debit_amount'];

					#### get total de-Subobligated fund ============== 
					$url = API_HOST_URL."get_all_reverse_fund_sub_obligate_to_obligate_total.php?ledger_type_id=".$ledger_type_id."&b_year=".$fund_beginning_fiscal_year."&e_year=".$fund_ending_fiscal_year."&fund_id=".$fund_id."&pe_id=".$pe_id."&ledger_type=".$ledger_type."";
					$de_subobligate_from_arr = requestByCURL($url);
					$total_de_subobligate_fund = $de_subobligate_from_arr['data'][0]['total_de_subobligate_amount'];

					### get total subobligated amount===============
					$url = API_HOST_URL."get_all_transaction_obligated_to_sub_obligate_total.php?ledger_type_id=".$ledger_type_id."&b_year=".$fund_beginning_fiscal_year."&e_year=".$fund_ending_fiscal_year."&fund_id=".$fund_id."&pe_id=".$pe_id."&ledger_type=".$ledger_type."";
					$sub_obligated_transaction_arr = requestByCURL($url); 
					$total_sub_obligated_amount = $sub_obligated_transaction_arr['data'][0]['total_sub_obligated_amount'];
					
					### get Invoice Paid for award only=============
					$ledger_type = str_replace(" ","_",$ledger_type);
		            ...
		            <tr class="parent-tbl">
		            	...
		            	<td>$<?php echo number_format($all_obligate_fund_gp_arr['data'][$count_pe]['total_amount']-$total_de_subobligate_fund); ?></td> 
		            	<td><?php echo '$'.number_format($unobligated_balance); ?></td>
		            	<td><?php echo '$'.number_format($total_sub_obligated_amount); ?></td>
		            	<td><?php echo '$'.number_format($total_de_subobligate_fund); ?></td>
		            	...
		            	<td colspan="12">
		            		<div style="padding:10px;">
		            			...
		            			<tr class="collapse in" style="background:#EBDEF0;">					
									...
									$url = API_HOST_URL."get_all_transaction_obligated_to_sub_obligate.php?ledger_type_id=".$ledger_type_id."&b_year=".$fund_beginning_fiscal_year."&e_year=".$fund_ending_fiscal_year."&fund_id=".$fund_id."&pe_id=".$pe_id."&ledger_type=".str_replace(' ', '_', $ledger_type)."";
									$alloted_transaction_arr = requestByCURL($url); 
									
									### get all reverse transaction from ==========
									$url = API_HOST_URL."get_all_reverse_fund_sub_obligate_to_obligate.php?ledger_type_id=".$ledger_type_id."&b_year=".$fund_beginning_fiscal_year."&e_year=".$fund_ending_fiscal_year."&fund_id=".$fund_id."&pe_id=".$pe_id."&ledger_type=".str_replace(' ', '_', $ledger_type)."";
									...
									$alloted_transaction_to_ac_arr = requestByCURL($url);
									...
									for($a=0; $a<count($transaction_arr['data']); $a++)
	            					{
	            						...
	            						if($transaction_arr['data'][$a]['credit_amount']>0)
	            						{
	            							...
	            							$debit_from = $transaction_arr['data'][$a]['op_id'].' ('.$transaction_arr['data'][$a]['ledger_type_dr'].')'.' - Commited';
	            						}
	            						else
	            						{
	            							$debit_from = $transaction_arr['data'][$a]['ledger_type_id'].' ('.$transaction_arr['data'][$a]['ledger_type'].')'.' - Obligated';
	            							$credit_in = $transaction_arr['data'][$a]['op_id'].' ('.$transaction_arr['data'][$a]['ledger_type_dr'].')'.' - Commited';
	            						} 
	            						?>
									...
									### loop for all transaction for sub obligate======
									...
									{  
	            						...
	            						<tr class="collapse in ">					
											...
											<td class="text_right"><?php echo '<div style="color:#2E9AFE;" title="Subobligated to '.$alloted_transaction_arr['data'][$c]['ledger_type_id_cr'].'"> - $'.number_format($alloted_transaction_arr['data'][$c]['debit_amount']).'</div>'; ?></td> 
	            							<td><?php echo $alloted_transaction_arr['data'][$c]['transaction_date']; ?></td>
	            						</tr>
	            						<?php
									}
									## alloted transaction for account payable =============== 
									for($c=0; $c<count($alloted_transaction_to_ac_arr['data']); $c++)
									{  
	            						?>
	            						<tr class="collapse in ">					
	            							<td><?php echo $alloted_transaction_to_ac_arr['data'][$c]['ledger_type_id_dr'].' ('.$alloted_transaction_to_ac_arr['data'][$c]['ledger_type_dr'].') - '.$alloted_transaction_to_ac_arr['data'][$c]['fund_status_dr']; ?></td>
	            							<td><?php echo $alloted_transaction_to_ac_arr['data'][$c]['ledger_type_id_cr'].' ('.$alloted_transaction_to_ac_arr['data'][$c]['ledger_type_cr'].') - '.$alloted_transaction_to_ac_arr['data'][$c]['fund_status_cr']; ?></td>
											<td class="text_right"><?php echo '<div style="color:#2E9AFE;" title="Paid to '.$alloted_transaction_to_ac_arr['data'][$c]['ledger_type_id_cr'].'"> - $'.number_format($alloted_transaction_to_ac_arr['data'][$c]['debit_amount']).'</div>'; ?></td> 
	            							<td><?php echo $alloted_transaction_to_ac_arr['data'][$c]['transaction_date']; ?></td>
	            						</tr>
	            						<?php
									}
            						...
            						?>
            						<tr class="collapse in ">
										<td><?php echo $reverse_from_arr['data'][$b]['ledger_type_id'].' ('.$reverse_from_arr['data'][$b]['ledger_type'].')'; ?> -Subobligate</td> 
										<td><?php echo $reverse_from_arr['data'][$b]['op_id'].' ('.$reverse_from_arr['data'][$b]['op_type'].')';?> - Obligated</td>
            							<td class="text_right"><?php echo '<div style="color:#E67E22;" title="DeSubObligated from '.$reverse_from_arr['data'][$b]['ledger_type_id'].' ('.$reverse_from_arr['data'][$b]['ledger_type'].')'.'"> $'.number_format($reverse_from_arr['data'][$b]['credit_amount']).'</div>'; ?></td> 
