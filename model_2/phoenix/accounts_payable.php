...
if(isset($_REQUEST['account_payable']))
{
	...
	$award_instrument_no = trim($_REQUEST['award_instrument_no']);
	...
	else
	{
		 $insert_account_payble_data = "insert into usaid_account_payble set
				...
				award_instrument_no = '".$award_instrument_no."'";
		...
		if($account_payble_new_id>0)
		{
			for($i=0; $i<count($activity_id); $i++)
			{
				...
				{	 
					...
					$result_account_payble_detail = $mysqli->query($insert_account_payble_detail);  
					
					## insert data in transction table=====================	
					$fund_all = explode('$', $fund[$i]);
					$narration=$fund_all[0];
					$amount = $fund_all[1]; //actual amount (real amount)
					...
					###insert query for credit amount into transaction detail table=======================
				 	$opn_clg_balance_arr = getClosingBalance($activity_id[$i],$beginning_fiscal_year,$ending_fiscal_year,$fund_id,'Project Activity',$pe_id,'Paid');
				 	...
				 	$insert_fund_transaction_detail_cr = "insert into usaid_fund_transaction_detail set
				 				...
				 				ledger_type = 'Project Activity',
				 				fund_status = 'Paid',
				 				transaction_year = '".$invoce_year."',
				 				ledger_type_id = '".$activity_id[$i]."'"; 
				 	...
				 	$opn_clg_balance_arr = getClosingBalance($activity_id[$i],$beginning_fiscal_year,$ending_fiscal_year,$fund_id,'Project Activity',$pe_id,'Obligate');
				 	$opening_balance = $opn_clg_balance_arr['opening_balance'];
				 	$closing_balance = $opn_clg_balance_arr['closing_balance'];
	...
	<div class="container-fluid manage-form">
		...
		<div class="new-form-content" style="width:54%; float:left">
			<div class="manage-funds"> 
					...
					<div class="form-group">
						<label class="col-md-4" for="Voucher ID">Project ID* :</label>
						<div class="col-md-8 show_project">						
							<select class="form-control project_id" name="project_id">
								<option value="">Select</option> 
							</select>
						</div>
						<div class="col-sm-8 aj_loader_project" style="display:none;">
		...
		<!----Section2------>
			...
			<!--	<form class="form-horizontal" role="form" method="post" action="">  -->
					<div class="form-group">
						<label class="col-md-4" for="Invoice Number">Activity ID* :</label>
						<div class="col-md-8 activity_id">
							<select class="form-control " name="">
								<option value="">Select</option> 	
							</select>
						</div>
						<div class="col-sm-8 aj_loader_activity" style="display:none;">
							 <img src="images/loading.gif">
						</div>
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						<label class="col-md-4" for="Invoice Description">Fund Strip:</label>
						<div class="col-md-8 fund" >
							<select name="fund_strip[]" class="form-control">
								<option value="">Select</option>
							</select>
						</div>
		            	...
		            	<td colspan="10">
							...
							?>		
            						<tr>
										...
										<td><?php echo $fetch_account_payble_detail['project_id']; ?></td> 
            							<td ><?php echo $fetch_account_payble_detail['activity_id']; ?></td> 
...
/*	$(document).ready(function() {
	...
	function getAwardInstrumentNumber()
	{
		...
		$.ajax({
		  ...
		  data: {vendor_id:vendor_id,project_id:project_id,operating_unit_id:operating_unit_id},
	...
	function getProjectId(elem)
	{
		...
		$('.aj_loader_activity').css('display','');
		var project_id = $(elem).val();		 
		$.ajax({
		  type: "POST",
		  url: "ajax_files/get_activity_drop_account.php",
		  data: {project_id:project_id},
		  success: function(data){
		  		$('.aj_loader_activity').css('display','none');
	...
	{
		var fund = $(elem).closest('.new-form-content').find('.fund');  
		$(fund).css('display','none');
		$(fund).next('.aj_loader_fund_strip').css('display','');
		var activity_id = $(elem).val();
		if(activity_id=='')
		{
...
/*	$('#datepicker2-icon').click(function(){
