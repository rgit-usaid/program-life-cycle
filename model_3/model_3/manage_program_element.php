...
if($project_id!=""){
	...
	$url = API_HOST_URL_PROJECT2."get_all_program_element.php";  
	...
	<div class="main-container container-fluid" id="main-content">
		<div>
			...
			<div class="tbl-block">
				...
				<div class="tbl-content-head">Manage Program Element</div>
				<div class="clear"></div>
			</div>
					...
					<div class="form-blk">
						<div class="add_program_element disp-none1">
							...
							<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
								<select name="program_element[]" multiple id="prgm_elem" style="display:none" class="form-control">
									<?php 
							...
							<div class="row">
								<div class="col-lg-8 col-md-8 col-sm-8 col-xs-12 text-right">
									...
									<button id="edit_prgm_elem" class="disp-none" type="button">Edit Program Element</button>
									...
									<form id="add_program_element" class="disp-none">
										...
										<table class="prgm_elems_info table table-striped disp-none text-left no-bdr" style="margin-top:30px;">
											...
											<tr class="save_btn_tr disp-none">
												<td colspan="3">&nbsp;</td>
												<td class="text-left">
													<button id="add_prgm_elems" type="button">Save</button>
												</td>
											</tr>
										</table>
									</form>
...
$('#add_prgm_elems').click(function(){
	...
	if(sum== 100 && $('#add_program_element').find('.invalid_ip').length==0 && total_filled_prgm_elems==0){
		...
		$.ajax({
			url:'<?php echo HOST_URL?>ajaxfiles/manage_program_element.php',
			data:form_data+'&add_program_element=add',
			type:'POST',
			success:function(data){
				var data = JSON.parse(data);
				var project_id =  $('#add_program_element').find('.project_id').val();
				...
				/*show msg*/		
				$('.add_prgm_elem_blk').find('.form-msg').addClass('usa-alert-success').removeClass('disp-none');
				$('.add_prgm_elem_blk').find('.usa-alert-heading').text('Success');
				$('.add_prgm_elem_blk').find('.usa-alert-text').text(data['msg']);
			
				
				get_all_project_program_elem(project_id);
				//window.location= "manage_program_element";
				setTimeout(function(){
					$('.add_prgm_elem_blk').find('.form-msg').addClass("disp-none").removeClass('usa-alert-success');
				},5000);
		...
		$('.add_prgm_elem_blk').find('.form-msg').addClass('usa-alert-error').removeClass('disp-none');
		$('.add_prgm_elem_blk').find('.usa-alert-heading').text('Error');
		$('.add_prgm_elem_blk').find('.usa-alert-text').text("Some error found...");
		
		setTimeout(function(){
			$('.add_prgm_elem_blk').find('.form-msg').addClass("disp-none").removeClass('usa-alert-error');
		},5000);
	}
	else if(total_filled_prgm_elems>0){
		$('.add_prgm_elem_blk').find('.form-msg').addClass('usa-alert-error').removeClass('disp-none');
		$('.add_prgm_elem_blk').find('.usa-alert-heading').text('Error');
		$('.add_prgm_elem_blk').find('.usa-alert-text').text("Percentage can't be zero.");
		...
		$('.add_prgm_elem_blk').find('.form-msg').addClass('usa-alert-error').removeClass('disp-none');
		$('.add_prgm_elem_blk').find('.usa-alert-heading').text('Error');
		$('.add_prgm_elem_blk').find('.usa-alert-text').text("Total Percentage must be 100%");
...
function get_all_project_program_elem(project_id){
	$.ajax({
		url:'<?php echo HOST_URL?>ajaxfiles/manage_program_element.php',
		type:'POST',
		data:{project_id:project_id,'list_data':'list_data'},
		success:function(data){
			var project_id =  $('#add_program_element').find('.project_id').val();
...
get_all_project_program_elem("<?php echo $project_id;?>");
...
function draw_graph(project_id){
	$.ajax({
		url:'<?php echo HOST_URL?>ajaxfiles/manage_program_element.php',
		type:'POST',
		data:{project_id:project_id,'graph_data':'graph_data'},
		success:function(data){
