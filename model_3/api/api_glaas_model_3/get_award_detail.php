<?php
include("config/config.inc.php");
include("includes/function.inc.php");
header('Content-type: application/json');
header("access-control-allow-origin: *");
## request for get award details============
if(isset($_REQUEST['award_number']) and $_REQUEST['award_number']!='')
{
    $data = array();
    $award_number =  trim($_REQUEST['award_number']);
    $select_data = "select * from usaid_requisition_award where award_number = '".$award_number."'";
    $result_data = $mysqli->query($select_data);
    $fetch_data = $result_data->fetch_array();
    if($fetch_data['id']!='')
    {
        $data['award_id'] = $fetch_data['id'];  
        $data['requisition_number'] = $fetch_data['requisition_number'];
        $data['award_number'] = $fetch_data['award_number'];
        $data['award_name'] = $fetch_data['award_name'];
        $data['vendor_id'] = $fetch_data['vendor_id'];
        $data['award_description'] = $fetch_data['award_description'];
		$data['amount'] = $fetch_data['amount'];
        $data['operating_unit_id'] = $fetch_data['operating_unit_id'];
        $data['employee_id'] = $fetch_data['employee_id'];
        $data['award_date'] = dateFormat($fetch_data['award_date']);  
        $data['start_performance_period'] = dateFormat($fetch_data['start_performance_period']);
        $data['end_performance_period'] = dateFormat($fetch_data['end_performance_period']);  
        $data['do_not_share'] = $fetch_data['do_not_share'];
        $data['implementing_mechanism_type'] = $fetch_data['implementing_mechanism_type'];
        deliverResponse(200,'Record Found',$data); 
    }
    else{
        deliverResponse(200,'No Record Found',NULL);
    }
}
else{
     deliverResponse(200,'Invalid Request',NULL);
}    
###function for deliver reponse on request===================
function deliverResponse($status,$status_msg,$data)
{
    $response['status'] = $status;
    $response['status_msg'] = $status_msg;
    $response['data'] = $data;
    $json_response = json_encode($response);
    echo $json_response;
}?>