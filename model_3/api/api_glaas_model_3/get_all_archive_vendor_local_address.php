...
if(isset($_REQUEST['archive_vendor_id']) and $_REQUEST['archive_vendor_id']!='')
{ 
    $data = array();
    $archive_vendor_id =  trim($_REQUEST['archive_vendor_id']);   
    $select_data = "select * from usaid_archive_vendor_local_address where archive_vendor_id='".$archive_vendor_id."'";
    $result_data = $mysqli->query($select_data);
    $i=0; 
    while($fetch_data = $result_data->fetch_array())
    {
        $data[$i]['archive_local_address_id'] = $fetch_data['id'];  
        $data[$i]['archive_vendor_id'] = $fetch_data['archive_vendor_id'];  
