...
$url = API_HOST_URL_PHOENIX."get_fund_by_activity.php?ledger_type_id=".$activity_id."&fund_status=Obligate";  
$activity_ob_fund  = requestByCURL($url);

$url = API_HOST_URL_PHOENIX."get_fund_by_activity.php?ledger_type_id=".$activity_id."&fund_status=Commit";  
$activity_cm_fund  = requestByCURL($url);

$url = API_HOST_URL_PHOENIX."get_all_account_payble_by_activity_id.php?activity_id=".$activity_id;  
...
for($i=0; $i<count($activity_ob_fund['data']);$i++){
	...
	if($byear>0){
		...
		$FY_year_arr[$byear]['obligated']= $activity_ob_fund['data'][$i]['credit_amount'];
...
for($i=0; $i<count($activity_cm_fund['data']);$i++){
	...
	if($byear>0){
		...
		$FY_year_arr[$byear]['fiscal_year']= $disp_label;
		$FY_year_arr[$byear]['b_year']= $byear;
		$FY_year_arr[$byear]['committed']= $activity_cm_fund['data'][$i]['credit_amount'];
...
//echo "sfy=$start_fyear<br/>";echo "sm=$start_fmonth<br/>";echo "em=$end_fmonth<br/>";
$tmp_sfm=0;$tmp_efm=0; //temp_sfm is temp_start-fiscal-month 
//echo "sy=$start_fyear<br/>";echo "ey=$end_fyear<br/>";
			...
			<div class="project-detail-blk table-container">
				...
				<div class="form-blk">
					...
					<div class="row project-detail-textarea">
						...
						<div id="mode_blk">
							<span class="bold">Choose One</span>&nbsp;
							<label><input type="radio"  value="monthly" checked="checked" name="mode"/> Month</label><br/>
							<label><input type="radio" value="quarterly" name="mode"/> Quarter</label>	
