...
if(isset($_REQUEST['ledger_type_id']) and $_REQUEST['ledger_type_id']!='')
{
    ...
    $select_fund_transaction = "select t.*,td1.ledger_type_id,td1.ledger_type,td1.fund_status
                        ...
                        where td2.ledger_type='Operating Unit' and (td1.ledger_type='DOAG' or td1.ledger_type='Award CLIN') and td1.credit_amount is not null and td2.ledger_type_id='".$ledger_type_id."'  and t.fund_id='".$fund_id."' and t.program_element_id='".$pe_id."' and td1.fund_status='Commit' ".$cond."  group by td1.ledger_type_id ";
    ...
    ### query for get paid amount to this operating unit============
    $select_fund_transaction = "select t.*,td2.ledger_type_id ,td1.ledger_type_id,sum(td2.debit_amount) as total_de_obliagted
                        from usaid_fund_transaction as t
                        left join usaid_fund_transaction_detail as td1 ON td1.transaction_id = t.id
                        ...
                        where td2.debit_amount is not null and td2.ledger_type_id in (".$ledger_type_id_str.") and t.fund_id='".$fund_id."' and t.program_element_id='".$pe_id."' and td1.fund_status='Commit' and td2.fund_status='Obligate' ".$cond."  group by td2.ledger_type_id ";
   
    $result_fund_transaction = $mysqli->query($select_fund_transaction) or die('Error'. $mysqli->error);
    $fetch_fund_transaction = $result_fund_transaction->fetch_array();
    $data['total_de_obliagted'] = $fetch_fund_transaction['total_de_obliagted']; // for commit in type id 
    if($data['total_de_obliagted']!=''){
