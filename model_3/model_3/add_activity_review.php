...
##==add new review==
if(isset($_REQUEST['add_new_review']) && isset($_REQUEST['activity_id']) && $_REQUEST['activity_id']!=""){
	...
	$project_output_score_description = $_REQUEST['activity_output_score_description'];
	$project_output_impact_weight = $_REQUEST['activity_output_impact_weight'];
	$project_output_performance = $_REQUEST['activity_output_performance'];
	$project_output_risk = $_REQUEST['activity_output_risk'];
	

	if($_REQUEST['review_id']=="")
	{
		##===find review for this date is unique or not==##
		$sel_review = "SELECT review_due_date FROM usaid_project_activity_monitoring WHERE activity_id='".$activity_id."' AND review_due_date='".$review_due_date."' AND review_type ='".$review_type."'";
		...
		if($exe_review->num_rows<=0){
			...
			$mysqli->autocommit(0);
			$ins ="INSERT INTO usaid_project_activity_monitoring set activity_id='".$activity_id."', review_type ='".$review_type."', review_due_date='".$review_due_date."', review_prompt_date='".$review_prompt_date."', ".$actual_review_date.", overall_score='".$overall_score."',annual_review_submission_comments='".$annual_review_submission_comments."',annual_review_approver_comments='".$annual_review_approver_comments."',annual_review_approval='".$annual_review_approval."',annual_review_approver='".$annual_review_approver."'";
			...
			if(!$exe){
				...
				##==insert review score==##
				$ins_opt_score = "INSERT INTO usaid_project_activity_monitoring_output_score(activity_monitoring_id, project_output_score_description, project_output_impact_weight, project_output_performance, project_output_risk) VALUES ";
				...
				if($exe_opt_score)
				{
				 ...
				 $mysqli->commit();
				 header("Location:add_activity_review");	
			    }
			    else
			    {
			      $_SESSION['form_msg']['msg_type'] ="error";
			...
			$_SESSION['form_msg']['msg_type'] ="error";
			$_SESSION['form_msg']['msg'] ="Activity Review already exitst for this date.";		
		}
	}
	else if($_REQUEST['review_id']!="")
	{
		...
		##===find review for this date is unique or not==##
		$sel_review = "SELECT review_due_date FROM usaid_project_activity_monitoring WHERE id!='".$review_id."' AND review_due_date='".$review_due_date."' AND review_type ='".$review_type."'";
		$exe_review = $mysqli->query($sel_review);
		if($exe_review->num_rows<=0)
		{
			##==update old review==#
			$mysqli->autocommit(0);
			$upd ="UPDATE usaid_project_activity_monitoring set review_type ='".$review_type."', review_due_date='".$review_due_date."', review_prompt_date='".$review_prompt_date."', ".$actual_review_date.", overall_score='".$overall_score."', annual_review_submission_comments='".$annual_review_submission_comments."', annual_review_approver_comments='".$annual_review_approver_comments."', annual_review_approval='".$annual_review_approval."', annual_review_approver='".$annual_review_approver."' WHERE id='".$review_id."'";
			...
			if(!$exe){
				...
				$_SESSION['form_msg']['msg']="Something went wrong.";		
					...
					if(count($rem_output_score_ids)>0)
					 {
						...
						$del_opt_score = "DELETE FROM usaid_project_activity_monitoring_output_score WHERE id IN(".$rem_output_score_ids_string.")";
					...
					{
						$rem_output_score_ids_string = implode(",",$rem_output_score_ids);
						$del_opt_score = "DELETE FROM usaid_project_activity_monitoring_output_score WHERE activity_monitoring_id  =".$review_id;
						$del_opt_score = $mysqli->query($del_opt_score);
						if(!$del_opt_score)
						{	 	
						  $_SESSION['form_msg']['msg_type'] ="error";
							...
							if($val!="" && $edit_row[$i]=="edit_row")
							 {
							   ...
							   $upd_opt_score = "UPDATE usaid_project_activity_monitoring_output_score set project_output_score_description='".$output_score_description."',
								 project_output_impact_weight='".$project_output_impact_weight[$i]."', project_output_performance='".$project_output_performance[$i]."', project_output_risk='".$project_output_risk[$i]."' WHERE id='".$val."'";
...
##==delete review==
if(isset($_REQUEST['delete_review']) && isset($_REQUEST['activity_id']) && $_REQUEST['activity_id']!="" && isset($_REQUEST['review_id']) && $_REQUEST['review_id']!="")
{
	$review_id = $_REQUEST['review_id'];
	$activity_id = $_REQUEST['activity_id'];
	$mysqli->autocommit(0);
	$del_review_op ="DELETE FROM usaid_project_activity_monitoring_output_score WHERE activity_monitoring_id='".$review_id."'";
	...
	if(!$exe_review_op){
		...
		$del_review ="DELETE FROM usaid_project_activity_monitoring WHERE id='".$review_id."' and activity_id='".$activity_id."'";
...
##==edit project review==
if(isset($_REQUEST['edit_review']) && isset($_REQUEST['activity_id']) && $_REQUEST['activity_id']!=""){
	$review_id = $_REQUEST['review_id'];
	$activity_id = $_REQUEST['activity_id'];
	$edit_mode = "edit_mode";
	##==get project_review_info
	$url = API_HOST_URL_PROJECT."get_all_project_activity_reviews.php?activity_id=".$activity_id."&review_id=".$review_id;    
    $activity_review_arr = requestByCURL($url);
	$review_type = $activity_review_arr['data'][0]['review_type'];
	$review_due_date = $activity_review_arr['data'][0]['review_due_date'];
	$review_prompt_date = $activity_review_arr['data'][0]['review_prompt_date'];
	$actual_review_date = $activity_review_arr['data'][0]['actual_review_date'];
	$overall_score = $activity_review_arr['data'][0]['overall_score'];
	$annual_review_submission_comments = $activity_review_arr['data'][0]['annual_review_submission_comments'];
	$annual_review_approval = $activity_review_arr['data'][0]['annual_review_approval'];
	$annual_review_approver = $activity_review_arr['data'][0]['annual_review_approver'];
	$annual_review_approver_comments = $activity_review_arr['data'][0]['annual_review_approver_comments'];
	
	$url = API_HOST_URL_PROJECT."get_project_activity_review_output_score.php?review_id=".$review_id; 
    $activity_review_op_score_arr = requestByCURL($url);
}
...
if($activity_id!=""){ 
    ...
    $url = API_HOST_URL_PROJECT."get_project_activity_upcoming_review.php?activity_id=".$activity_id."&review_type=annual_review";  
  	$activity_upcoming_ar = requestByCURL($url);

	##==get project upcoming project activity review info
	$url = API_HOST_URL_PROJECT."get_project_activity_upcoming_review.php?activity_id=".$activity_id."&review_type=project_activity_review"; 
  	$activity_upcoming_par = requestByCURL($url);
	if($activity_upcoming_ar['data']!==NULL){
		$ar_review_exits=true;
		$ar_due_date = str_replace("-","/",$activity_upcoming_ar['data']['review_due_date']);
		...
		$ar_prompt_date = str_replace("-","/",$activity_upcoming_ar['data']['review_prompt_date']);
	...
	if($activity_upcoming_par['data']!==NULL){
		$par_review_exits=true;
		$par_due_date = str_replace("-","/",$activity_upcoming_par['data']['review_due_date']);
		...
		$par_prompt_date = str_replace("-","/",$activity_upcoming_par['data']['review_prompt_date']);
		...
		<div>
			...
			<div class="tbl-block">
				<div class="tbl-caption">
					<div class="tbl-content-head">Manage Activity Monitoring</div>
					<div class="clear"></div>
				</div>
				<!--add project activity Review-->
				<div class="project-detail-blk table-container">
					...
					<div class="form-blk">
						...
						<div class="col-lg-8 col-md-8 col-sm-6 col-xs-12 <?php if($par_review_exits){?>disp-none<?php }?>">
							<div style="height:30px"></div>
							<div class="highlight_msg text-danger">No Activity Review Required.</div>
						</div>
						</div>
						<div class="gray-line"></div>
					</div>
					...
					<div class="form-blk">
									...
									<div class="form-blk">
										...
										<div class="add_output_scoring_blk">
											<table id="review_output_score_tbl" class="table table-bordered table-striped">
												...
												<?php if(isset($edit_mode) && count($activity_review_op_score_arr['data'])>0){
													  for($i=0;$i<count($activity_review_op_score_arr['data']);$i++){
													  $output_id = $activity_review_op_score_arr['data'][$i]['project_monitoring_output_score_id'];
												?>
												<input type="hidden" value="<?php echo $activity_review_op_score_arr['data'][$i]['project_monitoring_output_score_id'];?>" name="all_output_id[<?php echo $output_id;?>]" />
												<tr class="output_score_row">
													<td class="text-left">
														<input type="hidden" class="edit_row" value="" name="edit_row[]"/>
														<input type="hidden" value="<?php echo $activity_review_op_score_arr['data'][$i]['project_monitoring_output_score_id'];?>" name="actual_output_id[<?php echo $output_id?>]" />
														<input type="hidden" value="<?php echo $activity_review_op_score_arr['data'][$i]['project_monitoring_output_score_id'];?>" name="output_id[]" />
														<textarea class="form-control autoh_textarea" name="activity_output_score_description[]" readonly="readonly" rows="2"><?php echo $activity_review_op_score_arr['data'][$i]['project_output_score_description'];?></textarea>
													</td>
													<td>
														<input type="text" class="form-control impact_perc only_num"  name="activity_output_impact_weight[]" readonly="readonly" value="<?php echo $activity_review_op_score_arr['data'][$i]['project_output_impact_weight'];?>"/>
													</td>
													<td>
														<?php $perfm = $activity_review_op_score_arr['data'][$i]['project_output_performance'];?>
														<input type="hidden" name="activity_output_performance[]"  value="<?php echo $perfm;?>"/>
														<select class="form-control activity_output_performance_dpw" disabled="disabled">
													...
													<td><?php $risk= $activity_review_op_score_arr['data'][$i]['project_output_risk'];?>
														<input type="hidden" name="activity_output_risk[]"  value="<?php echo $risk;?>"/>
														<select class="form-control activity_output_risk_dpw" disabled="disabled">
										...
										<div class="row">
											...
											<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
												<div class="review_doc_rblk_out">
													...
													<div class="threshold">
															<div class="div_label">
															<input type="radio" name="annual_review_approval" value="Approve" class="review_req activity_status" <?php if(isset($edit_mode) && ($annual_review_approval=="Approve")){ echo "checked='checked'"; }?>/> <label> Approve </label> </div>
															<div style="height:10px;"></div>
															<div class="div_label">	
															<input type="radio" name="annual_review_approval" value="Reject" class="review_req activity_status" <?php if(isset($edit_mode) && ($annual_review_approval=="Reject")){ echo "checked='checked'"; }?> <?php if(isset($edit_mode) && ($annual_review_approval=="Approve")){?> disabled="disabled"  <?php }?>/> <label> Reject </label>
															</div>
						...
						<?php 
						$url = API_HOST_URL_PROJECT."get_all_project_activity_reviews.php?activity_id=".$activity_id."";  
							$project_review_arr = requestByCURL($url);
						?>
						<div class="review_listing_blk">
							<header><h3 class="form-blk-head">Project Activity Review List</h3></header>
							<div class="row">
								<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
									<table class="table table-bordered review-listing table-striped">
										...
										<tr class="data_tr">
											...
											<td class="text-center">
												...
												<form class="edit_review_form disp-inline" method="post">
													...
													<input type="hidden" value="<?php echo $project_review_arr['data'][$i]['activity_id'];?>" name="activity_id"/>
										...
										<?php }} else {?>
										<tr class="no_row_found">
											<td colspan="6" class="bold text-danger">No review found for this project</td>
										</tr>
										<?php }?>
									</table>
								</div>
...
<script>
	...
	$('#add_project_output').click(function(){
		var html = '<div class="opt-ip-outer"><textarea class="form-control opt-ip" name="activity_output_score_description[]"></textarea></div>';
	...
	$('#cancel_review').click(function(){
		...
		$('#add_review_form').find('.activity_status').removeAttr('disabled');
	...
	$('#add_output_score').click(function(){
		...
		var performance_dpw ='<select class="form-control" name="activity_output_performance[]"><option value="">Select Perfomance</option><option value="A">A</option><option value="B">B</option><option value="C">C</option><option value="D">D</option><option value="F">F</option></select>'; 
		var risk_dpw ='<select class="form-control" name="activity_output_risk[]"><option value="">Select Risk</option><option value="L">L</option><option value="M">M</option><option value="H">H</option></select>'; 
		var html= '<tr class="output_score_row"><td><input type="hidden" value="" name="output_id[]" /><textarea class="autoh_textarea" name="activity_output_score_description[]" rows="2"></textarea></td><td><input type="text" class="form-control impact_perc only_num"  name="activity_output_impact_weight[]"/></td><td>'+performance_dpw+'</td><td>'+risk_dpw+'</td><td class="text-center"><a class="btn btn-danger remove_output_score" title="Delete Review"><i class="fa fa-trash-o" aria-hidden="true"></i> </a></td></tr>';
	...
	$('#save_review').click(function(){
		...
		var blank_proj_output_score_desc =  $('#add_review_form').find('input[name="activity_output_score_description[]"]').filter(function(){
			return $(this).val()== "" ;
		}).length;
		
		var blank_proj_output_impact_weight =  $('#add_review_form').find('input[name="activity_output_impact_weight[]"]').filter(function(){
			return $(this).val()== "" ;
		}).length;
		...
		*/
		var blank_output_risk =  $('#add_review_form').find('select[name="activity_output_risk[]"]').filter(function(){
			return $(this).val()== "" ;
		}).length;
		...
		var activity_status_len = $('.activity_status:checked').length; 
		
		var invalid_proj_output_impact_weight =  $('#add_review_form').find('input[name="activity_output_impact_weight[]"]').filter(function(){
		...
		else if(activity_status_len>0 && $(".annual_review_approver").val()==""){
			error="error";
	...
	$('.activity_output_risk_dpw').change(function(){
		$(this).prev('input[name="activity_output_risk[]"]').val($(this).val());
	});
	
	$('.activity_output_performance_dpw').change(function(){
		$(this).prev('input[name="activity_output_performance[]"]').val($(this).val());
	});
