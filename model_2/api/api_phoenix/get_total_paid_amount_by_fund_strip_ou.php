...
if(isset($_REQUEST['ledger_type_id']) and $_REQUEST['ledger_type_id']!='')
{
	...
	#### this is used fro get all activity from commit or obligate screen of this operating unit============
    $select_fund_transaction = "select t.*,td1.ledger_type_id,td1.ledger_type,td1.fund_status
                        ...
                        where td2.ledger_type='Operating Unit' and td1.ledger_type='Project Activity' and td1.credit_amount is not null and td2.ledger_type_id='".$ledger_type_id."'  and t.fund_id='".$fund_id."' and t.program_element_id='".$pe_id."' and (td1.fund_status='Commit' or td1.fund_status='Obligate') ".$cond."  group by td1.ledger_type_id ";
   ...
   $select_fund_transaction = "select sum(td.credit_amount) as total_paid
                            from usaid_fund_transaction_detail td
                            left join usaid_fund_transaction t on td.transaction_id=t.id
                            where td.ledger_type='Project Activity' and td.fund_status='Paid' and td.credit_amount is not null and td.ledger_type_id in (".$ledger_type_id_str.") and t.fund_id='".$fund_id."' and t.program_element_id='".$pe_id."'".$cond."";
    $result_fund_transaction = $mysqli->query($select_fund_transaction) or die('Error'. $mysqli->error);
    $fetch_fund_transaction = $result_fund_transaction->fetch_array();
    $data['total_paid'] = $fetch_fund_transaction['total_paid']; // for commit in type id 
    if($data['total_paid']!=''){
