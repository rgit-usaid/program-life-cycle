...
function get_project_activity($project_id)
{
    ...
    {   
        ...
        while($fetch_data = $result_data->fetch_array())
        {
			...
			$data[$i]['procurement'] = getActivityProcurementData($fetch_data['activity_id']); //function for get all procurement
            $data[$i]['finance'] = getActivityFinanceData($fetch_data['activity_id']); //function for get all procurement  
            $i++;
        }
        return $data;
    }
...
{ 
    ...
    $url = API_HOST_URL_PHOENIX."get_operating_unit_by_id.php?operating_unit_id=".$operating_unit_id."";
    $ch = curl_init();
...
### get activity procurment data====================
function getActivityProcurementData($activity_id)
{
	$url = API_HOST_URL_PROJECT."get_all_procurement_detail_by_activity.php?activity_id=".$activity_id;
	$ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $url);                               
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    $output = curl_exec($ch);
...
### get activity finance data====================
function getActivityFinanceData($activity_id)
{
    $url = API_HOST_URL_PROJECT."get_finance_data_by_activity.php?activity_id=".$activity_id;
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $url);                               
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    $output = curl_exec($ch);
